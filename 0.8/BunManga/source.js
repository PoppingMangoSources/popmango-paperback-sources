...
var _Sources = (() => {
  ...
  // src/BunManga/BunManga.ts
  var BunManga_exports = {};
  __export(BunManga_exports, {
    BunManga: () => BunManga,
    BunMangaInfo: () => BunMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // common/Dates.ts
  var RELATIVE = /^\s*(?:about\s+)?(\d+(?:\.\d+)?)\s*([a-z]+)\.?\s*(?:ago)?\s*$/i;
  var UNIT_MS = {
    second: 1e3,
    minute: 60 * 1e3,
    hour: 60 * 60 * 1e3,
  ...
  function parseDate(raw) {
    ...
    return parseKeyword(text) ?? parseRelative(text) ?? parseTimestamp(text) ?? parseNamedMonth(text) ?? parseNumeric(text) ?? parseNative(text);
  }
  ...
  function parseRelative(text) {
    ...
    const unit = UNIT_MS[rawUnit] !== void 0 ? rawUnit : UNIT_ALIASES[rawUnit];
    const unitMs = unit === void 0 ? void 0 : UNIT_MS[unit];
    if (!Number.isFinite(amount) || unitMs === void 0) {
      return void 0;
    }
  ...
  function parseTimestamp(text) {
    ...
    return new Date(text.length === 10 ? value * 1e3 : value);
  }
  function parseNamedMonth(text) {
    ...
    const parts = monthFirst !== null ? { month: monthFirst[1], day: monthFirst[2], year: monthFirst[3] } : dayFirst !== null ? { month: dayFirst[2], day: dayFirst[1], year: dayFirst[3] } : void 0;
    if (parts === void 0) {
      return void 0;
    }
  ...
  // src/BunManga/models.ts
  var DOMAIN = "https://bunmanga.com";
  var PAGE_SIZE = 12;
  var SECTIONS = {
    POPULAR: "popular",
    TOP_DAILY: "top_daily",
  ...
  // src/BunManga/network.ts
  var IMAGE_URL2 = /\.(avif|gif|jpe?g|jxl|png|svg|webp)([/?#]|$)/i;
  var BunMangaInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
  ...
  function buildSearchUrl(request) {
    const url = URL(request.title !== void 0 && request.title.length > 0 ? DOMAIN : `${DOMAIN}/?s`).setQueryItem("post_type", "wp-manga");
    if (request.title !== void 0 && request.title.length > 0) {
      url.setQueryItem("s", request.title);
    }
    if (request.sortBy !== void 0 && request.sortBy !== "relevance") {
    ...
    if (request.adult !== void 0) {
      url.setQueryItem("adult", request.adult);
    }
    if (request.statuses !== void 0 && request.statuses.length > 0) {
  ...
  function fetchLoadMorePage(page, queryVars, referer) {
  ...
  function fetchChapterList(mangaId) {
    return Application.fetchDocument({
      url: URL(DOMAIN).addPathComponent("manga").addPathComponent(mangaId).addPathComponent("ajax").addPathComponent("chapters").build(),
      method: "POST",
      headers: {
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
  ...
  function fetchReaderPage(mangaId, chapterId) {
    return Application.fetchDocument({
      url: URL(DOMAIN).addPathComponent("manga").addPathComponent(mangaId).addPathComponent(chapterId).setQueryItem("style", "list").build()
    });
  }
  function formValue(value) {
    return encodeURIComponent(value).replace(/%20/g, "+");
  }
  function appendFormValues(entries, key, value) {
  ...
  function loadMoreBody(page, queryVars) {
    let variables;
    try {
  ...
  // src/BunManga/parsers.ts
  var UNSAFE_ID = /[^a-zA-Z0-9._\-@()[\]%?#+=/&:]/g;
  var ORIGINAL_SCRIPT = /[぀-ヿ㐀-鿿가-힯]/;
  var ADULT_GENRES = /* @__PURE__ */ new Set(["adult", "hentai", "smut"]);
  var MATURE_GENRES = /* @__PURE__ */ new Set(["mature", "soft yaoi", "soft yuri", "yaoi", "yuri"]);
  ...
  function toAbsoluteUrl(value) {
    const url = Application.decodeHTMLEntities(value ?? "").replace(/\s+/g, "").trim();
    if (url.length === 0) {
      return "";
    }
    if (url.startsWith("//")) {
  ...
  function imageUrlFrom(image) {
    const srcset = image.attr("srcset")?.split(",").map((entry) => {
      const [url, width] = entry.trim().split(/\s+/);
      return { url, width: Number.parseInt(width ?? "", 10) || 0 };
    }).filter((entry) => entry.url !== void 0).sort((left, right) => right.width - left.width)[0]?.url;
  ...
  function pickOriginalTitle(value, primaryTitle) {
    const titles = splitTitles(value, primaryTitle);
    return titles.find((title) => ORIGINAL_SCRIPT.test(title)) ?? titles[0];
  }
  function splitTitles(value, primaryTitle) {
    return value.split(/\s*[;/]\s*/).map(cleanText).filter(
      (title, index, values) => title.length > 0 && title.toLowerCase() !== primaryTitle.toLowerCase() && values.indexOf(title) === index
    );
  }
  ...
  function chapterTitle(value) {
    const remainder = cleanText(
      cleanText(value).replace(/\b(?:chapter|chap|ch\.?)[\s_-]*\d+(?:\.\d+)?/i, "").replace(/^[-:\s]+/, "")
    );
    return remainder.length > 0 ? remainder : void 0;
  }
  function parseListingChapter(container) {
  ...
  function tileItem($, element, titleSelector, imageSelector) {
    const item = $(element);
  ...
  function parseMangaList($) {
    ...
    for (const element of $(".c-tabs-item__content").toArray()) {
      ...
      seen.add(mangaId);
      const genres = item.find(".mg_genres .summary-content a").map((_, link) => cleanText($(link).text())).toArray().filter((genre) => genre.length > 0);
      ...
      items.push({
        ...
        status: status.length > 0 ? status : void 0,
        chapter: parseListingChapter(item),
  ...
  function widgetByHeading($, selector, heading) {
    return $(selector).filter((_, element) => heading.test(cleanText($(element).find(".heading").first().text()))).first();
  }
  function parsePopular($) {
    return widgetByHeading($, ".widget-manga-popular-slider", /^popular$/i).find(".slider__item").toArray().flatMap((element) => {
      const item = tileItem($, element, ".post-title a", ".slider__thumb img");
      return item === void 0 ? [] : [item];
    });
  }
  function parseTopDaily($) {
    return widgetByHeading($, ".widget-manga-recent", /^top daily$/i).find(".popular-item-wrap").toArray().flatMap((element) => {
      const item = tileItem($, element, ".widget-title a", ".popular-img img");
      return item === void 0 ? [] : [item];
    });
  }
  function parseLatestUpdates($) {
    return $(".c-blog-listing .page-item-detail").toArray().flatMap((element) => {
  ...
  function parseLoadMoreQueryVars($) {
    for (const element of $("script").toArray()) {
      const match = $(element).text().match(/var\s+__madara_query_vars\s*=\s*(\{[^\n;]+\})\s*;/);
      if (match !== null) {
        return match[1];
      }
    }
  ...
  function toDiscoverItem(item) {
    return {
      ...
      subtitle: joinDetails([item.chapter?.title, item.rating === void 0 ? void 0 : `\u2605 ${item.rating}`])
    };
  }
  ...
  function labelledContent($, label) {
    for (const element of $(".post-content_item").toArray()) {
      const item = $(element);
      const heading = cleanText(item.find(".summary-heading").first().text()).replace(/\(s\)$/i, "").toLowerCase();
      ...
      return [
        {
          ...
          publishDate: parseDate(
            item.find(".chapter-release-date .c-new-tag").first().attr("title") ?? item.find(".chapter-release-date").text()
          )
        }
      ];
    });
  ...
  function parseChapterDetails($, chapter) {
    const pages = $(".reading-content .page-break img").toArray().map((element) => imageUrlFrom($(element))).filter((url) => url.length > 0);
    if (pages.length === 0) {
      throw new Error(
  ...
  var BunManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: SECTIONS.TOP_DAILY, title: "Top Daily", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST_UPDATES, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.RELEVANCE, title: "Relevance", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.TOP_RATED, title: "Top Rated", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.TRENDING, title: "Trending", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
    ...
    async searchPage(request, metadata) {
      ...
      const hasMore = queryVars !== void 0 && items.length > 0 && (total !== void 0 ? page * PAGE_SIZE < total : page === 1 ? hasLoadMore(document) : items.length === PAGE_SIZE);
      return {
        items,
        metadata: hasMore ? { page: page + 1, queryVars, total } : void 0
      };
    }
    /** Turns the chosen filter tags into the site's own search parameters. */
    toSearchRequest(query) {
  ...
  return __toCommonJS(BunManga_exports);
})();
