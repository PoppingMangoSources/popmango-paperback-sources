...
var _Sources = (() => {
  ...
  // src/GalaxyManga/GalaxyManga.ts
  var GalaxyManga_exports = {};
  __export(GalaxyManga_exports, {
    GalaxyManga: () => GalaxyManga,
    GalaxyMangaInfo: () => GalaxyMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/GalaxyManga/models.ts
  var DOMAIN = "https://galaxymanga.io";
  var MANGA_DIR = "manga";
  var NEXT_PAGE_SELECTOR = "div.pagination .next, div.hpage .r, a.next.page-numbers, a.r";
  var VISIBLE_SECTIONS_KEY = "galaxymanga.visibleSections";
  ...
  var SECTION_DEFINITIONS = {
    [SECTIONS.POPULAR]: { id: SECTIONS.POPULAR, title: "Popular", type: "featured" /* featured */ },
  ...
  var TRENDING_RANGE = "wpop-weekly";
  var TRENDING_RANGES = [
  ...
  // src/GalaxyManga/network.ts
  var IMAGE_URL2 = /\.(avif|gif|jpe?g|jxl|png|svg|webp)([/?#]|$)/i;
  var GalaxyMangaInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
  ...
  function directoryUrl(page, request = {}) {
  ...
  function fetchDirectoryPage(page, request = {}) {
  ...
  function fetchChapterPage(chapterId) {
    return Application.fetchDocument({ url: chapterUrl(chapterId) });
  }

  // src/GalaxyManga/parsers.ts
  var UNSAFE_ID = /[^a-zA-Z0-9._\-@()[\]%?#+=/&:]/g;
  var ADULT_GENRES = /* @__PURE__ */ new Set(["adult", "hentai", "pornographic", "erotica"]);
  var MATURE_GENRES = /* @__PURE__ */ new Set(["ecchi", "mature", "smut", "yaoi", "yuri", "adult romance"]);
  ...
  function imageUrlFrom(image) {
    ...
    return toAbsoluteUrl(
      image.attr("data-lazy-src") ?? image.attr("data-src") ?? image.attr("data-cfsrc") ?? srcset ?? image.attr("src")
    );
  }
  ...
  function parseDate2(value) {
    ...
    return Number.isNaN(direct.getTime()) ? void 0 : direct;
  }
  function chapterLabel(chapter) {
    if (chapter === void 0 || chapter.length === 0) {
      return void 0;
    }
    const number = chapter.match(/(\d+(?:\.\d+)?)/)?.[1];
    return number !== void 0 ? `Ch. ${number}` : chapter;
  }
  ...
  function parseCard($, element) {
    ...
    const title = cleanText(
      unit.find(".bigor .tt a").first().text() || unit.find(".bigor .tt").first().text() || image.attr("title") || link.attr("title")
    );
    if (title.length === 0) {
      return void 0;
    }
    ...
    return {
      ...
      chapter: chapter.length > 0 ? chapter : void 0,
      rating: rating.length > 0 ? rating : void 0,
      typeName: typeName.length > 0 ? typeName : void 0,
      genres: []
    };
  }
  function parseCards($) {
  ...
  function parseWidgetCards($, heading) {
    const cards = [];
    const seen = /* @__PURE__ */ new Set();
    widgetByHeading($, heading).find(".bsx").each((_, element) => {
      const card = parseCard($, element);
      if (card === void 0 || seen.has(card.mangaId)) {
        return;
      }
  ...
  function parseLatestCards($, heading) {
    const cards = [];
    const seen = /* @__PURE__ */ new Set();
    widgetByHeading($, heading).find(".bsx").each((_, element) => {
      const card = parseCard($, element);
      ...
      cards.push({
        ...card,
        chapterId: href.length > 0 ? parseChapterId(href) : void 0,
        chapterName: chapterName.length > 0 ? chapterName : void 0,
        publishDate: parseDate2(latest.find(".fivtime").text())
      });
    });
    return cards;
  }
  function parseTrendingCards($, range) {
    const cards = [];
    const seen = /* @__PURE__ */ new Set();
    $(`.serieslist.wpop.${range} ul li, #wpop-items .${range} ul li`).each((_, element) => {
      ...
      cards.push({
        ...
        rating: rating.length > 0 ? rating : void 0,
        rank: Number.isFinite(rank) ? rank : void 0,
        genres: item.find('a[href*="/genres/"]').toArray().map((genre) => cleanText($(genre).text())).filter((genre) => genre.length > 0)
      });
    });
    return cards;
  }
  function parseGenreOptions($) {
    const genres = [];
    const seen = /* @__PURE__ */ new Set();
  ...
  function parseMangaDetails($, mangaId) {
    ...
    const scope = details.length > 0 ? details : $.root();
    ...
    const secondaryTitles = cleanText(scope.find(ALT_NAME_SELECTOR).first().text()).split(/[,;|]/).map((value) => value.trim()).filter((value) => value.length > 0 && value.toLowerCase() !== primaryTitle.toLowerCase());
    const genreTags = [];
  ...
  function toDiscoverItem(card) {
    ...
    return {
      ...
      subtitle: joinDetails([
        card.rank === void 0 ? void 0 : `#${card.rank}`,
        chapterLabel(card.chapter),
        card.rating === void 0 ? void 0 : `\u2605 ${card.rating}`,
        typeName === void 0 ? void 0 : TYPE_COUNTRIES[typeName] ?? card.typeName
      ])
    };
  }
  function toLatestItem(card) {
    if (card.chapterId === void 0) {
      return void 0;
    }
    return {
      mangaId: card.mangaId,
  ...
  function toSearchResultItem(card) {
    return {
      mangaId: card.mangaId,
      title: card.title,
      imageUrl: card.imageUrl,
  ...
  // src/GalaxyManga/GalaxyManga.ts
  var GalaxyMangaInfo = sourceInfo({
    name: "Galaxy Manga",
  ...
  var GalaxyManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getSearchResultItems(query, metadata) {
      ...
      return {
        ...
        metadata: document(NEXT_PAGE_SELECTOR).length > 0 ? { page: page + 1 } : void 0
      };
    }
  ...
  return __toCommonJS(GalaxyManga_exports);
})();
