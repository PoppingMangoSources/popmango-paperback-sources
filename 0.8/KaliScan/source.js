...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  function switchRow(id, options) {
    return App.createDUISwitch({
      id,
      label: options.label,
      value: App.createDUIBinding({
        get: async () => options.get(),
        set: async (value) => options.set(value === true)
      })
    });
  }
  function labelRow(id, label, value) {
  ...
  var DOMAIN = MIRRORS[0]?.id ?? "https://kaliscan.com";
  var MIRROR_IDS = MIRRORS.map((mirror) => mirror.id);
  var BASE_URL_KEY = "kaliscan.baseUrl";
  var ACTIVE_BASE_URL_KEY = "kaliscan.activeBaseUrl";
  var FAILOVER_KEY = "kaliscan.automaticFailover";
  var SETTINGS_KEYS = [BASE_URL_KEY, ACTIVE_BASE_URL_KEY, FAILOVER_KEY];
  var SECTIONS = {
    POPULAR: "popular",
    HOT: "hot-updates",
    LATEST: "latest",
  ...
  function setActiveBaseUrl(origin) {
    if (binding !== void 0 && binding.failover() && MIRROR_IDS.includes(origin)) {
      binding.setActive(origin);
    }
  }
  function mirrorOrigin(url) {
    const origin = originOf(url);
    return MIRROR_IDS.includes(origin) ? origin : void 0;
  }
  ...
  // src/KaliScan/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif|bmp)(\?|#|$)/i;
  var RETRYABLE = /* @__PURE__ */ new Set([403, 408, 500, 502, 503, 504, 521, 522, 523, 524]);
  function completeMobileSafariUserAgent(userAgent2) {
    if (!/\b(?:iPhone|iPad|iPod)\b/.test(userAgent2) || /\bSafari\//.test(userAgent2)) {
      return userAgent2;
    }
    const os = /\bOS (\d+)[_.](\d+)/.exec(userAgent2);
    const version = os !== null ? `${os[1]}.${os[2]}` : "18.0";
    const withVersion = /\bVersion\//.test(userAgent2) ? userAgent2 : userAgent2.replace(/\sMobile\//, ` Version/${version} Mobile/`);
  ...
  var KaliScanInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      const isImage = IMAGE_URL2.test(request.url);
      ...
      return {
        ...
        headers: {
          ...
          accept: isImage ? "image/avif,image/webp,image/apng,image/png,image/svg+xml,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
        }
      };
    }
  ...
  async function fetchHtml(url) {
    ...
    const origins = automaticFailover() ? [requested, selectedBaseUrl(), ...MIRROR_IDS].filter(
      (origin, index, values) => origin !== void 0 && origin.length > 0 && values.indexOf(origin) === index
    ) : requested !== void 0 ? [requested] : [];
    const candidates = origins.length > 0 && requested !== void 0 ? origins.map((origin) => url.replace(requested, origin)) : [url];
    let lastError;
  ...
  function contentRatingForGenres(genres, isAdult = false) {
    if (isAdult) {
      return "ADULT" /* ADULT */;
    }
    const normalised = genres.map((genre) => genre.trim().toLowerCase());
    return normalised.some((genre) => ADULT_GENRES.includes(genre)) ? "ADULT" /* ADULT */ : "MATURE" /* MATURE */;
  }
  function chapterNumberFrom(value) {
    const match = /chapter[.\s-]*(\d+(?:\.\d+)?)/i.exec(value);
    return match !== null ? parseFloat(match[1] ?? "0") : void 0;
  }
  var RELATIVE_UNITS = [
    ["second", 1e3],
    ["minute", 6e4],
  ...
  function parseSiteDate(value) {
    ...
    if (text.includes("ago")) {
      ...
      return unit !== void 0 ? new Date(Date.now() - count * unit) : void 0;
    }
    const ymd = /^(\d{4})-(\d{1,2})-(\d{1,2})(?:[ t](\d{1,2}):(\d{2})(?::(\d{2}))?)?$/.exec(text);
    const date = ymd !== null ? new Date(
      Date.UTC(
        Number(ymd[1]),
        Number(ymd[2]) - 1,
        Number(ymd[3]),
    ...
    return date.getTime() > Date.now() ? /* @__PURE__ */ new Date() : date;
  }
  function coverFrom(element) {
    const img = element.find("img").first();
    const source = img.attr("data-src") ?? img.attr("data-lazy-src") ?? img.attr("data-cfsrc") ?? img.attr("srcset")?.split(",")[0]?.trim().split(/\s+/)[0] ?? img.attr("src") ?? "";
  ...
  function isEmbeddedData(value) {
    ...
    if (strings.some((key) => record[key] !== void 0 && record[key] !== null && typeof record[key] !== "string")) {
      return false;
    }
    if (record.is_adult !== void 0 && record.is_adult !== null && typeof record.is_adult !== "number" && typeof record.is_adult !== "boolean") {
      return false;
    }
    if (record.genres === void 0 || record.genres === null) {
      return true;
    }
    return Array.isArray(record.genres) && record.genres.every((genre) => {
      if (genre === null || typeof genre !== "object" || Array.isArray(genre)) {
        return false;
      }
      const name = genre.name;
      return name === void 0 || name === null || typeof name === "string";
    });
  }
  function embeddedData(item) {
    ...
    try {
      ...
      return isEmbeddedData(value) ? value : void 0;
    } catch {
      return void 0;
    }
  }
  function cardFrom($, element) {
    ...
    if (mangaSlugFromUrl(url) === void 0) {
      return void 0;
    }
    ...
    const markupGenres = item.find(".genres span, .genres a").toArray().map((genre) => cleanText($(genre).text())).filter((genre) => genre.length > 0);
    const embeddedGenres = (data?.genres ?? []).map((genre) => genre.name ?? "").filter((genre) => genre.length > 0);
    return {
      url,
      title,
      cover: data?.cover !== void 0 ? absoluteUrl(data.cover) : coverFrom(item),
      latestChapter: cleanText(item.find(".latest-chapter").first().text()) || cleanText(chapterLink.text()) || void 0,
      latestChapterUrl: chapterLink.attr("href"),
      views: cleanText(data?.views ?? item.find(".views span, .views").first().text()) || void 0,
      rating: (data?.rating ?? cleanText(item.find(".rating .score, .rating").first().text())).replace(
        /[^\d.]/g,
        ""
      ) || void 0,
  ...
  function collectCards(html, selector, requireCover) {
  ...
  function parseHotCells(html) {
    return collectCards(html, ".trending-item", true);
  }
  function ratingText(card) {
    const value = card.rating !== void 0 ? parseFloat(card.rating) : NaN;
    return isFinite(value) && value > 0 ? value.toFixed(1) : void 0;
  }
  function toFeaturedItems(cards) {
      ...
      return [
        {
          ...
          subtitle: [rating !== void 0 ? `${rating} \u2605` : "", card.views ?? ""].filter((part) => part.length > 0).join(" \u2022 ") || void 0
        }
      ];
    });
  }
  function toRankedItems(cards, detail, ranked = true) {
    return cards.flatMap((card, index) => {
      ...
      const rating = ratingText(card);
      const lead = detail === "chapter" ? card.latestChapter : detail === "views" ? card.views && `${card.views} views` : rating && `${rating} \u2605`;
      return [
        {
          ...
          imageUrl: card.cover,
          title: card.title,
          subtitle: [ranked ? `#${index + 1}` : "", lead ?? ""].filter((part) => part.length > 0).join(" \u2022 ") || void 0
        }
      ];
    });
  }
  function toLatestItems(cards) {
    return cards.flatMap((card) => {
      const slug = mangaSlugFromUrl(card.url);
      if (slug === void 0) {
        return [];
      }
      const chapNum = card.latestChapter !== void 0 ? chapterNumberFrom(card.latestChapter) : void 0;
      const chapterSlug = card.latestChapterUrl !== void 0 ? (card.latestChapterUrl.split("/").pop() ?? "").replace(/[?#].*$/, "") : chapNum !== void 0 ? `chapter-${chapNum}` : void 0;
      if (chapterSlug === void 0 || chapterSlug.length === 0) {
        return [];
      }
      const rating = ratingText(card);
      return [
        {
          ...
          title: card.title,
          subtitle: [
            chapNum !== void 0 ? `Ch. ${chapNum}` : card.latestChapter ?? "",
            rating !== void 0 ? `${rating} \u2605` : ""
          ].filter((part) => part.length > 0).join(" \u2022 ") || void 0,
          publishDate: card.updatedAt !== void 0 ? parseSiteDate(card.updatedAt) : void 0
        }
      ];
    });
  }
  function toSearchResultItems(cards) {
    return cards.flatMap((card) => {
      const slug = mangaSlugFromUrl(card.url);
      if (slug === void 0) {
        return [];
      ...
      return [
        {
          ...
          title: card.title,
          imageUrl: card.cover,
          subtitle: [rating !== void 0 ? `${rating} \u2605` : "", card.genres[0] ?? ""].filter((part) => part.length > 0).join(" \u2022 ") || void 0,
  ...
  function parseMangaDetails(html, mangaId) {
    ...
    const authors = $('.detail .meta > p > strong:contains("Authors") ~ a').toArray().map((element) => cleanText($(element).text())).filter((author) => author.length > 0);
    const genres = $('.detail .meta > p > strong:contains("Genres") ~ a').toArray().map(
      (element) => cleanText(
    ...
    const thumbnailUrl = $('meta[property="og:image"]').attr("content") ?? cover.attr("data-src") ?? cover.attr("src") ?? "";
    const summary = $(".summary p, .summary .content").toArray().map((element) => cleanText($(element).text())).filter((text) => text.length > 0 && !/^you are reading\b/i.test(text)).join("\n\n");
    const secondaryTitles = cleanText($(".detail h2, .name h2").first().text()).split(/[,;]/).map((alias) => alias.trim()).filter((alias) => alias.length > 0 && alias.toLowerCase() !== title.toLowerCase());
    const seen = /* @__PURE__ */ new Set();
    const tags = genres.flatMap((genre) => {
    ...
    const rating = score.length > 0 && scored > 0 ? Math.min(1, Math.max(0, scored / 5)) : void 0;
    return {
      mangaId,
      mangaInfo: {
        primaryTitle: title,
  ...
  function parseChapterList(html, sourceManga) {
    ...
    const chapters = rows.flatMap((element, index) => {
      ...
      const chapterId = url.split("/").filter((part) => part.length > 0).pop()?.replace(/[?#].*$/, "");
      if (chapterId === void 0 || chapterId.length === 0) {
        return [];
      }
      ...
      return [
        {
          ...
          title: name.length > 0 && !/^chapter[.\s-]*[\d.]+$/i.test(name) ? name : void 0,
          volume: 0,
          sortingIndex: rows.length - index,
          publishDate: dateText.length > 0 ? parseSiteDate(dateText) : void 0
        }
      ];
    });
  ...
  function parseChapterPages(html, chapter) {
    const $ = Application.loadDocument(html);
    let pages = $("#chapter-images img, .chapter-image, .chapter-image img, .chapter-lazy-image").toArray().map((element) => {
      const image = $(element);
      return image.attr("data-src") ?? image.attr("data-lazy-src") ?? image.attr("data-cfsrc") ?? image.attr("src") ?? "";
    ...
    if (pages.length === 0) {
      ...
      if (list !== void 0) {
        ...
        pages = list.split(",").map((path) => path.trim()).filter((path) => path.length > 0).map((path) => {
  ...
  function mangaUrl(mangaId) {
    return `${baseUrl()}/manga/${decodeSlugId(mangaId)}`;
  }

  // src/KaliScan/KaliScan.ts
  var KaliScanInfo = sourceInfo({
    name: "KaliScan",
    ...
    capabilities: ["chapters" /* CHAPTERS */, "homePage" /* HOME_PAGE */, "cloudflare" /* CLOUDFLARE */, "settings" /* SETTINGS */]
  });
  var KaliScan = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
      ...
      return [
        {
          id: "mirror",
          header: "Mirrors",
          footer: "All four domains carry the same catalogue. Failover moves to another one only when the chosen site is blocked, unreachable, or returning an error.",
          rows: [
            selectRow("base_url", {
              ...
              set: (value) => {
                const mirror = value[0] !== void 0 && MIRROR_IDS.includes(value[0]) ? value[0] : DOMAIN;
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.POPULAR, title: "Popular", type: "featured" /* featured */ },
        { id: SECTIONS.HOT, title: "Hot Updates", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.NEWEST, title: "Newest", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.REVIEWS, title: "Top Reviewed", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
    async getDiscoverSectionItems(section, metadata) {
      switch (section.id) {
        ...
        case SECTIONS.LATEST:
          return this.getLatest(metadata);
    ...
    async getFilterSections() {
      return [
        ...
        },
        {
          ...
          tags: GENRE_MODE_OPTIONS.map(
            (option) => filterTag(FILTERS.GENRE_MODE, option.id, option.title)
          )
        },
        {
          id: FILTERS.STATUS,
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const url = range !== void 0 ? `${baseUrl()}/top/${encodeURIComponent(range)}?page=${page}` : this.searchUrl(query, included, page);
