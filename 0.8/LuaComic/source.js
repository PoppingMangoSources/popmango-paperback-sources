...
var _Sources = (() => {
  ...
  // src/LuaComic/LuaComic.ts
  var LuaComic_exports = {};
  __export(LuaComic_exports, {
    LuaComic: () => LuaComic,
    LuaComicInfo: () => LuaComicInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/LuaComic/models.ts
  var DOMAIN = "https://luacomic.org";
  var API_URL = "https://api.luacomic.org";
  var PAGE_SIZE = 20;
  var PAID_CHAPTER_SUFFIX = "#paid";
  var SHOW_PAID_KEY = "luacomic.showPaidChapters";
  var SHOW_ADULT_KEY = "luacomic.showAdultContent";
  var SETTINGS_KEYS = [SHOW_PAID_KEY, SHOW_ADULT_KEY];
  var SECTIONS = {
    POPULAR: "popular",
    FEATURED: "featured",
  ...
  var ADULT_GENRES = ["adult", "smut", "mature", "ecchi", "hentai", "yaoi", "yuri"];

  // src/LuaComic/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif|bmp|svg)(\?|#|$)/i;
  var LuaComicInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      return {
        ...request,
        headers: {
          ...
          accept: IMAGE_URL2.test(request.url) ? "image/avif,image/webp,image/apng,image/png,image/svg+xml,*/*;q=0.8" : "application/json, text/html, */*",
  ...
  async function fetchTags() {
    const raw = await Application.fetchJSON({
      url: URL(API_URL).addPathComponent("tags").build()
    });
    const tags = Array.isArray(raw) ? raw : raw.data ?? [];
    return tags.filter((tag) => Boolean(tag.name) && tag.id !== null && tag.id !== void 0).map((tag) => ({ id: String(tag.id), value: String(tag.name).trim() }));
  }
  function fetchQuery(request) {
    const url = URL(API_URL).addPathComponent("query").setQueryItem("page", request.page).setQueryItem("perPage", PAGE_SIZE).setQueryItem("series_type", "Comic").setQueryItem("adult", request.adult ? "true" : "false").setQueryItem("orderBy", request.orderBy ?? "created_at").setQueryItem("status", request.status).setQueryItem("query_string", request.search);
    const numericIds = (request.genres ?? []).filter((id) => /^\d+$/.test(id));
    if (numericIds.length > 0) {
      url.setQueryItem("tags_ids", `[${numericIds.join(",")}]`);
    }
    return Application.fetchJSON({ url: url.build() });
  }
  function fetchTrending(range) {
    return Application.fetchJSON({
      url: URL(API_URL).addPathComponent("trending").setQueryItem("type", range).build()
    });
  }
  function fetchAllChapters(slug) {
    return Application.fetchJSON({
  ...
  function fetchSeriesPage(slug) {
    return fetchText(`${DOMAIN}/series/${slug}`);
  }
  function fetchChapterPage(seriesSlug, chapterSlug) {
    return fetchText(`${DOMAIN}/series/${seriesSlug}/${chapterSlug}`);
  }
  function mangaUrl(slug) {
    return `${DOMAIN}/series/${slug}`;
  }

  // src/LuaComic/parsers.ts
  ...
  function mapStatus(status) {
    ...
    return value.length > 0 ? value.charAt(0).toUpperCase() + value.slice(1).toLowerCase() : void 0;
  }
  ...
  function ratingToUnit(value) {
    if (value === null || value === void 0 || !isFinite(value)) {
      return void 0;
    }
    return Math.min(1, Math.max(0, value / 5));
  }
  function tagNames(series) {
    return (series.tags ?? []).map((tag) => typeof tag === "string" ? tag : tag.name ?? "").map((name) => name.trim()).filter((name) => name.length > 0);
  }
  function contentRatingForSeries(series) {
    return tagNames(series).some(isAdultGenre) ? "ADULT" /* ADULT */ : "MATURE" /* MATURE */;
  }
  function yearOf(value) {
    return /^(\d{4})/.exec(value ?? "")?.[1];
  }
  function chapterCount(meta) {
    const raw = meta?.chapters_count;
    const parsed = typeof raw === "number" ? raw : parseInt(String(raw ?? ""), 10);
    return isFinite(parsed) && parsed > 0 ? parsed : void 0;
  }
  function flightPayload(html) {
    return html.replace(/"\]\)\s*;?\s*(?:<\/script>\s*<script>\s*)?self\.__next_f\.push\(\[1,\s*"/g, "").replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }
  ...
  function resolveFlightText(payload, value) {
    if (value === null || value === void 0 || value.length === 0) {
      return void 0;
    }
    ...
    return (boundary === -1 ? chunk : chunk.slice(0, boundary)).trim() || void 0;
  }
  function resolveSeriesText(payload, series) {
    return { ...series, description: resolveFlightText(payload, series.description) ?? null };
  }
  function parseHomePage(html) {
    const payload = flightPayload(html);
    return {
      banners: extractArray(payload, "banners").map((banner) => ({
        ...banner,
        series: banner.series ? resolveSeriesText(payload, banner.series) : banner.series
      })),
      recommended: extractArray(payload, "series").map(
        (series) => resolveSeriesText(payload, series)
      ),
      editors: extractArray(payload, "pinned_series").map(
        (series) => resolveSeriesText(payload, series)
      )
    };
  }
  function parseMetaContent(html, property) {
    ...
    return match?.[1] !== void 0 && match[1].length > 0 ? Application.decodeHTMLEntities(match[1]) : void 0;
  }
  function parseJsonNumber(payload, key) {
    const match = new RegExp(`"${key}"\\s*:\\s*(\\d+)`).exec(payload);
    if (match === null) {
      return void 0;
    }
    const value = parseInt(match[1] ?? "", 10);
    return isFinite(value) ? value : void 0;
  }
  function parseJsonString(payload, key) {
  ...
  function parseSeriesPage(html, slug) {
    ...
    return {
      ...
      tags: extractArray(payload, "tags"),
      meta: { chapters_count: parseJsonNumber(payload, "chapters_count") ?? null }
    };
  }
  function toPopularItems(entries) {
    return entries.map((series) => {
      const subtitle = [
        series.rating !== null && series.rating !== void 0 && isFinite(series.rating) ? `\u2605 ${series.rating.toFixed(1)}` : "",
  ...
  function toBannerItems(banners) {
    return banners.flatMap((banner) => {
      const series = banner.series;
      if (series === null || series === void 0 || !series.series_slug) {
        return [];
      }
      ...
      return [
        {
          ...
          title: cleanText(series.title),
          subtitle: views !== null && views !== void 0 ? `${formatCount(views)} views` : void 0
        }
      ];
    });
  }
  function toRecommendedItems(entries) {
    return entries.map((series) => {
      const subtitle = [mapStatus(series.status) ?? "", yearOf(series.created_at) ?? "", cleanText(series.author)].filter((part) => part.length > 0).join(" \u2022 ");
      return {
  ...
  function chapterIsPaid(chapter) {
    return (chapter.price ?? 0) > 0;
  }
  function newestChapter(series) {
    return [...series.free_chapters ?? []].filter((chapter) => !chapterIsPaid(chapter)).sort(
      (left, right) => new Date(right.created_at ?? 0).getTime() - new Date(left.created_at ?? 0).getTime()
    )[0];
  }
  function toLatestItems(entries) {
      ...
      return [
        {
          ...
          imageUrl: series.thumbnail ?? "",
          title: cleanText(series.title),
          subtitle: cleanText(chapter.chapter_name) || void 0,
          publishDate: isNaN(publishDate.getTime()) ? void 0 : publishDate
        }
      ];
    });
  }
  function toRankedItems(entries) {
    return entries.map((entry, index) => {
      ...
      return {
        ...
        subtitle: [`#${index + 1}`, chapters !== void 0 ? `${chapters} ch` : ""].filter((part) => part.length > 0).join(" \u2022 ")
      };
    });
  }
  function toSearchResultItems(entries) {
    return entries.map((series) => ({
      mangaId: encodeSlugId(series.series_slug),
      title: cleanText(series.title),
      imageUrl: series.thumbnail ?? "",
  ...
  function toTrendingSearchItems(entries) {
    return entries.map((entry, index) => {
      ...
      return {
        ...
        title: entry.title,
        imageUrl: entry.thumbnail ?? "",
        subtitle: [`#${index + 1}`, isFinite(chapters) ? `${chapters} ch` : ""].filter((part) => part.length > 0).join(" \u2022 "),
        contentRating: "MATURE" /* MATURE */
      };
    });
  }
  function parseMangaDetails(series) {
    const primaryTitle = cleanText(series.title) || "Untitled";
    const secondaryTitles = [];
    const seen = /* @__PURE__ */ new Set([primaryTitle.toLowerCase()]);
    ...
    const tags = genres.map((name) => ({ id: sanitizeId(name), title: name }));
    const views = series.total_views;
    return {
      mangaId: encodeSlugId(series.series_slug),
      mangaInfo: {
        ...
        additionalInfo: views !== null && views !== void 0 && isFinite(views) ? { views: formatCount(views) } : void 0
      }
    };
  }
  ...
  function parseChapterList(chapters, sourceManga, showPaid) {
    return chapters.filter((chapter) => showPaid || !chapterIsPaid(chapter)).sort((left, right) => chapterNumberOf(left) - chapterNumberOf(right)).map((chapter, index) => {
      ...
      return {
        ...
        sourceManga,
        title: paid ? title.length > 0 ? `${title} \u{1F512}` : "\u{1F512}" : title,
        chapNum: chapterNumberOf(chapter),
        volume: 0,
        langCode: "\u{1F1EC}\u{1F1E7}",
        sortingIndex: index,
        publishDate: chapter.created_at ? new Date(chapter.created_at) : void 0
      };
    });
  }
  function parseChapterPages(html, chapter) {
    ...
    if (start !== -1) {
      ...
      if (slice !== void 0) {
        try {
          pages = JSON.parse(slice).filter((url) => typeof url === "string");
        } catch {
          pages = [];
        }
      }
    ...
    pages = pages.map((url) => url.replace(/%3A/gi, ":").replace(/%2F/gi, "/").replace(/ /g, "%20")).filter((url) => url.includes("media.luacomic.org") || url.includes("/uploads/series/"));
    if (pages.length === 0) {
  ...
  var LuaComic = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
      ...
      return [
        {
          id: "content",
          header: "What to show",
          footer: "Paid chapters have to be unlocked on the website before they will open. Adult series are left out of browsing and search unless asked for.",
          rows: [
            switchRow("show_paid", {
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.POPULAR, title: "Most Popular", type: "featured" /* featured */ },
        { id: SECTIONS.RECOMMENDED, title: "Recommended", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.FEATURED, title: "Featured", type: "featured" /* featured */ },
        { id: SECTIONS.EDITORS, title: "Editor's Choice", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
      ...
      switch (section.id) {
        ...
        case SECTIONS.LATEST: {
          ...
          return {
            ...
            metadata: (data.meta?.last_page ?? 1) > page ? { page: page + 1 } : void 0
          };
        }
        default:
          return { items: [] };
    ...
    // -----------------------------------------------------------------
    getHomePage() {
      this.homePage ??= fetchHomePage().then(parseHomePage).catch((error) => {
    ...
    getGenres() {
      this.genres ??= fetchTags().then((tags) => tags.length > 0 ? tags : FALLBACK_GENRES).catch((error) => {
        if (error instanceof CloudflareError) {
          this.genres = void 0;
          throw error;
        }
  ...
  return __toCommonJS(LuaComic_exports);
})();
