...
var _Sources = (() => {
  ...
  // src/StoneScape/StoneScape.ts
  var StoneScape_exports = {};
  __export(StoneScape_exports, {
    StoneScape: () => StoneScape,
    StoneScapeInfo: () => StoneScapeInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/StoneScape/models.ts
  var DOMAIN = "https://stonescape.xyz";
  var API_URL = `${DOMAIN}/api`;
  var FALLBACK_IMAGE_URL = `${DOMAIN}/logo.png`;
  var PAGE_SIZE = 20;
  var CONTENT_TYPE = "manhwa";
  var SHOW_LOCKED_CHAPTERS_KEY = "stonescape_show_locked_chapters";
  var SETTINGS_KEYS = [SHOW_LOCKED_CHAPTERS_KEY];
  var SECTIONS = {
    FEATURED: "featured",
    LATEST: "latest"
  };
  ...
  // src/StoneScape/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif|svg)(\/|\?|#|$)/i;
  var StoneScapeInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      ...
      return {
        ...
        headers: {
          ...
          accept: request.headers.accept ?? (isImage ? "image/avif,image/webp,image/apng,image/*,*/*;q=0.8" : "application/json, text/plain, */*"),
    ...
    async interceptResponse(request, response, body) {
      ...
      const challenged = isCloudflareChallenge(response.status, response.headers, text) || response.status === 403 && request.url.startsWith(DOMAIN);
      if (challenged) {
  ...
  function buildSeriesUrl(query) {
    return apiUrl("series").setQueryItem("page", query.page).setQueryItem("limit", query.limit ?? 20).setQueryItem("contentType", CONTENT_TYPE).setQueryItem("genres", query.genres !== void 0 ? query.genres.join(",") : void 0).setQueryItem("status", query.status).setQueryItem("search", query.search).setQueryItem("sort", query.sort === "latest" ? void 0 : query.sort).build();
  }
  function fetchSeries(query) {
    return Application.fetchJSON({ url: buildSeriesUrl(query) });
  }
  function fetchBanner() {
    return Application.fetchJSON({ url: apiUrl("banner-config").build() });
  }
  function fetchPopular(period, limit) {
    return Application.fetchJSON({
      url: apiUrl("series", "popular").setQueryItem("period", period).setQueryItem("contentType", CONTENT_TYPE).setQueryItem("limit", limit).build()
    });
  }
  function fetchGenres() {
    return Application.fetchJSON({ url: apiUrl("genres").build() });
  }
  function fetchSeriesDetails(slug) {
    return Application.fetchJSON({ url: apiUrl("series", "by-slug", slug).build() });
  }
  ...
  // src/StoneScape/parsers.ts
  var UNSAFE_ID = /[^a-zA-Z0-9._\-@()[\]%?#+=/&:]/g;
  var LOCKED_PREFIX = "locked:";
  function encodeMangaId(slug) {
    return slug.replace(UNSAFE_ID, "-");
  }
  function decodeMangaId(mangaId) {
    try {
  ...
  function toAbsoluteUrl(value) {
    ...
    const absolute = (path.startsWith("http://") || path.startsWith("https://") ? path : path.startsWith("//") ? `https:${path}` : `${DOMAIN}${path.startsWith("/") ? "" : "/"}${path}`).replace(/(\.(?:avif|gif|jpe?g|jxl|png|svg|webp))\/+(?=([?#]|$))/i, "$1");
    return /^https?:\/\/[^/\s?#]+(?:[/?#]|$)/i.test(absolute) ? absolute : "";
  }
  function staticImageUrl(...values) {
  ...
  function formatMediaType(series) {
  ...
  function contentRatingForGenres(genreNames) {
    const genres = genreNames.map((name) => name.trim().toLowerCase());
    if (genres.some(
      (name) => name === "adult" || name === "hentai" || name === "shotacon" || name === "smut" || name === "yaoi"
    )) {
      return "ADULT" /* ADULT */;
    }
  ...
  function formatViews(views) {
    if (views === void 0 || !isFinite(views)) {
      return void 0;
    }
    if (views >= 1e6) {
  ...
  function parseMangaList(series) {
    return series.map((item) => {
      const parsedViews = item.totalViews === void 0 || item.totalViews === null ? NaN : parseInt(String(item.totalViews), 10);
      return {
        ...
        summary: decodeText(item.description) || void 0,
        author: decodeText(item.author) || void 0,
        status: mapStatus(item.publicationStatus),
        rating: item.averageRating === void 0 || item.averageRating === null || !isFinite(item.averageRating) ? void 0 : item.averageRating,
        views: isFinite(parsedViews) ? parsedViews : void 0,
  ...
  function toFeaturedItem(item) {
    const subtitle = [
      item.mediaType,
      item.rating === void 0 ? "" : `\u2605 ${item.rating.toFixed(1)}`,
  ...
  function toSearchResultItem(item) {
    const subtitle = [
      ...
      item.rating === void 0 ? "" : `Rating ${item.rating.toFixed(1)}`,
      formatViews(item.views) ?? ""
    ].filter((part) => part.length > 0).join(" \u2022 ");
    return {
      mangaId: item.mangaId,
  ...
  function toChapterUpdateItem(series) {
    const chapter = series.latestChapter;
    if (chapter === void 0 || chapter === null || !chapter.chapterId || !chapter.chapterNumber) {
      return void 0;
    }
    return {
      ...
      publishDate: parseDate2(chapter.createdAt ?? series.lastChapterUploadedAt)
    };
  }
  ...
  function parseMangaDetails(series) {
    ...
    const tags = genres.map((genre) => ({
      ...
      title: genre.replace(/[-_]/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").replace(/\b\w/g, (character) => character.toUpperCase())
    }));
    return {
      mangaId: encodeMangaId(series.slug),
      mangaInfo: {
        primaryTitle,
        secondaryTitles: originalTitle.length > 0 && originalTitle.toLowerCase() !== primaryTitle.toLowerCase() ? [originalTitle] : [],
        ...
        rating: series.averageRating === void 0 || series.averageRating === null || !isFinite(series.averageRating) ? void 0 : Math.min(1, Math.max(0, series.averageRating / 5)),
  ...
  function parseChapterList(chapters, sourceManga, showLocked) {
    return chapters.filter((chapter) => showLocked || !chapterIsLocked(chapter)).map((chapter) => ({ chapter, number: parseFloat(chapter.chapterNumber) })).sort((left, right) => {
      ...
      return (parseDate2(left.chapter.createdAt)?.getTime() ?? 0) - (parseDate2(right.chapter.createdAt)?.getTime() ?? 0);
      ...
      return {
        ...
        title: locked ? rawTitle.length > 0 ? `${rawTitle} \u{1F512}` : "\u{1F512}" : rawTitle || void 0,
        volume: 0,
        sortingIndex: index,
        publishDate: parseDate2(chapter.releaseDate ?? chapter.createdAt)
      };
    });
  }
  function decodeChapterId(chapterId) {
  ...
  function parseChapterPages(response, chapter) {
    const source = response.pages !== void 0 && response.pages.length > 0 ? response.pages : response.images ?? [];
    const pages = [...source].sort(
      (left, right) => (left.pageNumber ?? Number.MAX_SAFE_INTEGER) - (right.pageNumber ?? Number.MAX_SAFE_INTEGER)
    ).map((page) => toAbsoluteUrl(page.url)).filter((url) => /^https?:\/\/\S+$/i.test(url));
    if (pages.length === 0) {
  ...
  var StoneScape = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.FEATURED, title: "Featured Series", type: "featured" /* featured */ },
        { id: SECTIONS.LATEST, title: "Latest Releases", type: "chapterUpdates" /* chapterUpdates */ }
      ];
    }
        ...
        case SECTIONS.LATEST: {
          ...
          return {
            items: response.data.flatMap((series) => {
              ...
              return item === void 0 || !/^https?:\/\/\S+$/i.test(item.imageUrl) ? [] : [item];
            })
          };
        }
        default:
      ...
      return [
        ...
        },
        {
          ...
          tags: (await this.getGenres()).map(
            (genre) => filterTag(FILTERS.GENRE, genre.id, genre.title)
          )
        },
        {
          id: FILTERS.STATUS,
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const response = await fetchSeries({
        page,
        limit: PAGE_SIZE,
        genres: includedGenres.length > 0 ? includedGenres : void 0,
      ...
      const series = excludedGenres.length === 0 ? response.data : response.data.filter((item) => {
      ...
      return {
        ...
        metadata: page < response.pagination.totalPages ? { page: page + 1 } : void 0
      };
    }
    ...
    async getFeatured() {
      ...
      return parseMangaList(featured).filter((item) => item.imageUrl.length > 0).map(toFeaturedItem);
    }
    getGenres() {
      this.genres ??= fetchGenres().then((response) => response.genres.map((genre) => ({ id: genre.slug, title: genre.label }))).catch((error) => {
        this.genres = void 0;
        throw error;
      });
      return this.genres;
  ...
  function normalisedGenre(value) {
  ...
  return __toCommonJS(StoneScape_exports);
})();
