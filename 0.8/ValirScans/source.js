...
var _Sources = (() => {
  ...
  // src/ValirScans/ValirScans.ts
  var ValirScans_exports = {};
  __export(ValirScans_exports, {
    ValirScans: () => ValirScans,
    ValirScansInfo: () => ValirScansInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/ValirScans/models.ts
  var DOMAIN = "https://valirscans.org";
  var LOCKED_CHAPTER_PREFIX = "locked:";
  var NOVEL_TYPE = "Novel";
  var BASE_URL_KEY = "valirscans.baseUrl";
  var SHOW_PAID_CHAPTERS_KEY = "valirscans.showPaidChapters";
  var SETTINGS_KEYS = [BASE_URL_KEY, SHOW_PAID_CHAPTERS_KEY];
  var FILTERS = {
    SORT: "sort",
    GENRE: "genre",
    TAG: "tag",
  ...
  var MIN_CHAPTERS_FIELD = "minChapters";
  var MAX_CHAPTERS_FIELD = "maxChapters";
  var SORT_OPTIONS = [
  ...
  var DISCOVER_SECTIONS = [
    { id: SECTIONS.FEATURED, title: "Top Featured", type: "featured" /* featured */ },
    { id: SECTIONS.MOST_POPULAR, title: "Most Popular", type: "simpleCarousel" /* simpleCarousel */ },
    { id: SECTIONS.LATEST_COMICS, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
    { id: SECTIONS.POPULAR_TODAY, title: "Popular Today", type: "prominentCarousel" /* prominentCarousel */ },
    { id: SECTIONS.EDITORS_PICKS, title: "Editors' Picks", type: "prominentCarousel" /* prominentCarousel */ },
    { id: SECTIONS.NEW_SERIES, title: "New Series", type: "simpleCarousel" /* simpleCarousel */ }
  ];

  // src/ValirScans/site.ts
  var read;
  function bindBaseUrl(reader) {
    read = reader;
  }
  function baseUrl() {
    return read?.() ?? DOMAIN;
  ...
  // src/ValirScans/network.ts
  var CHALLENGE_PROMPT_INTERVAL = 6e4;
  var ValirScansInterceptor = class extends PaperbackInterceptor {
    constructor() {
      super(...arguments);
  ...
  function fetchPage(url, rsc = false) {
    return Application.fetchText({ url, method: "GET", headers: rsc ? { rsc: "1" } : void 0 });
  }
  function fetchHomePage() {
    return fetchPage(`${baseUrl()}/`);
  }
  function fetchBrowsePage(request) {
    const url = URL(`${baseUrl()}/series`).setQueryItem("page", request.page);
    const query = (request.query ?? "").trim();
    if (query.length > 0) {
      url.setQueryItem("q", query);
    }
    if (request.sort !== void 0) {
  ...
  function fetchSeriesPage(mangaId, page = 1) {
    return fetchPage(`${baseUrl()}/series/${mangaId}${page > 1 ? `?page=${page}` : ""}`);
  }
  function fetchChapterPage(mangaId, chapterId) {
  ...
  function sliceBalanced(payload, start) {
    let depth = 0;
    let inString = false;
    let escaped = false;
  ...
  function extractByMarker(payload, marker, keepMarker = false) {
    const values = [];
  ...
  function isNovel(series) {
    return (series.type ?? "").toUpperCase().includes(NOVEL_TYPE.toUpperCase());
  }
  function comicsOnly(list) {
    return list.filter((series) => !isNovel(series));
  }
  function toMangaId(series) {
    return `comic/${series.urlSlug ?? series.slug}`;
  }
  function toContentRating(series) {
    return series.isMature === true ? "MATURE" /* MATURE */ : "EVERYONE" /* EVERYONE */;
  }
  ...
  function statSubtitle(series, rank) {
    const lead = rank !== void 0 ? `#${rank}` : series.rating !== void 0 ? `\u2605 ${series.rating.toFixed(1)}` : "";
    const parts = [lead, series.viewCount !== void 0 ? `${formatCount(series.viewCount)} views` : ""].filter(
      (part) => part.length > 0
    );
  ...
  function parseHomeSections(html) {
    ...
    const seriesLists = extractByMarker(payload, '"series":').filter(
      (list) => Array.isArray(list) && list.length > 0
    );
    return {
      featured: comicsOnly(extractByMarker(payload, '"initialSlides":')[0] ?? []),
      ...
      editorsPicks: comicsOnly(
        seriesLists.find((list) => list[0] !== void 0 && "viewCount" in list[0] && !("createdAt" in list[0])) ?? []
      ),
      latestUpdates: comicsOnly(
        seriesLists.find((list) => list[0] !== void 0 && "lastChapterAt" in list[0]) ?? []
      ),
      // Named for the component rather than its contents; the shelf carries
      // comics too, and the novels among them are dropped here.
      popularToday: comicsOnly(extractByMarker(payload, '"novels":')[0] ?? []),
      // Ranked cards are emitted in rank order, one prop per card.
      mostPopular: comicsOnly(
        extractByMarker(payload, '"novel":').filter(
          (series) => Boolean(series.slug) && Boolean(series.title)
        )
      )
    };
  }
  function parseFilterTaxonomy(html) {
    const payload = decodeFlightPayload(html);
    const pick = (key) => (extractByMarker(payload, `"${key}":`).filter((list) => Array.isArray(list) && Boolean(list[0]?.name) && Boolean(list[0]?.slug)).sort((left, right) => right.length - left.length)[0] ?? []).flatMap(
      (entry) => entry.name !== void 0 && entry.slug !== void 0 ? [{ id: sanitizeId(entry.slug), title: cleanText(entry.name) }] : []
    );
    return { genres: pick("genres"), tags: pick("tags") };
  }
  function parseBrowsePage(html) {
    const payload = decodeFlightPayload(html);
    const series = extractByMarker(payload, '"initialSeries":')[0];
    if (series === void 0) {
  ...
  function parseSeriesPage(html) {
    const payload = decodeFlightPayload(html);
    const page = extractByMarker(payload, '{"series":', true).find(
  ...
  function parseMangaDetails(page, mangaId) {
    const series = page.series;
    const secondaryTitles = [
      ...new Set(
        [series.altTitle, series.originalTitle, ...series.aliases ?? []].map((title) => cleanText(title)).filter((title) => title.length > 0 && title !== cleanText(series.title))
      )
    ];
    const genres = (series.genres ?? []).flatMap((entry) => {
      const genre = entry.genre ?? entry;
      const title = genre.name !== void 0 ? cleanText(genre.name) : genre.slug !== void 0 ? toTitleCase(genre.slug) : "";
      if (title.length === 0) {
        return [];
      }
      return [{ id: sanitizeId(genre.slug ?? title), title }];
    });
    const tags = (series.tags ?? []).filter((tag) => tag.name !== void 0).map((tag) => ({ id: sanitizeId(tag.slug ?? tag.name), title: cleanText(tag.name) }));
    const tagGroups = [];
    if (genres.length > 0) {
    ...
    return {
      ...
      mangaInfo: {
        ...
        bannerUrl: series.bannerImage !== null && series.bannerImage !== void 0 ? toAbsoluteUrl(series.bannerImage) : void 0,
        ...
        author: cleanText(series.author) || void 0,
        artist: cleanText(series.artist) || void 0,
        rating: series.rating !== void 0 ? Math.min(1, Math.max(0, series.rating / 10)) : void 0,
        tagGroups: tagGroups.length > 0 ? tagGroups : void 0
      }
    };
  }
  function chapterIsLocked(chapter) {
    return chapter.isLocked === true;
  }
  function chapterTitle(chapter) {
    const title = cleanText(chapter.title).replace(/^chapter\s+\d+(?:\.\d+)?(?:\s*[-:]\s*)?/i, "").trim();
    if (!chapterIsLocked(chapter)) {
      return title;
    }
    return title.length > 0 ? `${title} \u{1F512}` : "\u{1F512}";
  }
  function parseChapters(seriesPages, sourceManga, showPaidChapters) {
    const seen = /* @__PURE__ */ new Set();
    return seriesPages.flatMap((page) => page.chapters ?? []).filter((chapter) => showPaidChapters || !chapterIsLocked(chapter)).filter((chapter) => {
      if (seen.has(chapter.number)) {
        return false;
      }
      seen.add(chapter.number);
      return true;
    }).sort((left, right) => left.number - right.number).map((chapter, index) => ({
      chapterId: chapterIsLocked(chapter) ? `${LOCKED_CHAPTER_PREFIX}${chapter.number}` : String(chapter.number),
      sourceManga,
      langCode: "\u{1F1EC}\u{1F1E7}",
      chapNum: chapter.number,
      title: chapterTitle(chapter),
      volume: 0,
      publishDate: chapter.publishedAt ? new Date(chapter.publishedAt) : void 0,
      sortingIndex: index
    }));
  }
  function parseChapterDetails(html, chapter) {
    const payload = decodeFlightPayload(html);
    const data = extractByMarker(payload, '"chapter":').find(
    ...
    const pages = (data.pages ?? []).slice().sort((left, right) => left.pageNumber - right.pageNumber).map((page) => toAbsoluteUrl(page.imageUrl)).filter((url) => url.length > 0);
    if (pages.length > 0) {
      return { id: chapter.chapterId, mangaId: chapter.sourceManga.mangaId, pages };
    }
    throw new Error(`Chapter ${chapter.chapterId} has no page images.`);
  }
  function toFeaturedItems(list) {
  ...
  function toCarouselItems(list, type, ranked = false) {
    return list.map((series, index) => ({
      type,
      mangaId: toMangaId(series),
      imageUrl: toAbsoluteUrl(series.coverImage),
      title: cleanText(series.title),
      subtitle: statSubtitle(series, ranked ? index + 1 : void 0)
    }));
  }
  function toChapterUpdateItems(list) {
      ...
      return [
        {
          ...
          publishDate: date ? new Date(date) : void 0
        }
      ];
    });
  }
  function toSearchResultItem(series) {
    const subtitle = [
      series.rating !== void 0 ? `\u2605 ${series.rating.toFixed(1)}` : "",
      series.type !== void 0 ? toTitleCase(series.type.replace(/_/g, " ")) : ""
    ].filter((part) => part.length > 0).join(" \xB7 ");
    return {
      mangaId: toMangaId(series),
      title: cleanText(series.title),
      imageUrl: toAbsoluteUrl(series.coverImage),
  ...
  var ValirScans = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    getSettingsSections() {
      return [
        ...
        },
        {
          ...
          footer: `Point the extension somewhere else if the site moves. Leave this empty for the default (${DOMAIN}). An address that does not look like a website is ignored.`,
          rows: [
            inputRow("base_url", {
              label: "Address",
    ...
    async getDiscoverSectionItems(section, metadata) {
      if (section.id === SECTIONS.NEW_SERIES) {
        ...
        return {
          ...
          metadata: browse.hasMore ? { page: page + 1 } : void 0
        };
      }
    ...
    async getChapterList(sourceManga) {
      ...
      const laterPages = await Promise.all(
        Array.from(
          ...
          (_, index) => fetchSeriesPage(sourceManga.mangaId, index + 2).then(parseSeriesPage)
        )
      );
    ...
    // -----------------------------------------------------------------
    getTaxonomy() {
      this.taxonomy ??= fetchBrowsePage({ page: 1 }).then(parseFilterTaxonomy).then((taxonomy) => taxonomy.genres.length > 0 ? taxonomy : { ...taxonomy, genres: GENRES }).catch((error) => {
        this.taxonomy = void 0;
        throw error;
      });
    ...
    getHome() {
      this.home ??= fetchHomePage().then(parseHomeSections).finally(() => {
        this.home = void 0;
      });
      return this.home;
    }
  ...
  return __toCommonJS(ValirScans_exports);
})();
