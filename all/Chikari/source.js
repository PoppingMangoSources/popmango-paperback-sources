...
var _Sources = (() => {
  ...
  // src/Chikari/Chikari.ts
  var Chikari_exports = {};
  __export(Chikari_exports, {
    Chikari: () => Chikari,
    ChikariInfo: () => ChikariInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/Chikari/models.ts
  var DOMAIN = "https://chikari.moe";
  ...
  var DEFAULT_CONTENT_RATINGS = ["safe", "suggestive"];
  var DEFAULT_CONTENT_TYPES = ["manga", "manhwa", "manhua"];
  ...
  var SECTION_DEFINITIONS = {
    [SECTIONS.FEATURED]: { id: SECTIONS.FEATURED, title: "Popular", type: "featured" /* featured */ },
    [SECTIONS.RECENTLY_UPDATED]: {
      id: SECTIONS.RECENTLY_UPDATED,
      title: "Recently Updated",
      type: "chapterUpdates" /* chapterUpdates */
    }
  };
  var SECTION_IDS = Object.values(SECTIONS);
  var SECTION_OPTIONS = SECTION_IDS.map((id) => ({
    id,
  ...
  var TYPE_OPTIONS = CONTENT_TYPE_OPTIONS;
  var FILTERS = {
    SORT: "sort",
    PERIOD: "period",
    GENRE: "genre",
  ...
  var ChikariInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      return {
        ...request,
        headers: {
          ...request.headers,
          accept: request.url.startsWith(API_URL) ? "application/json, text/plain, */*" : request.headers.accept ?? "*/*",
          origin: DOMAIN,
          referer: `${DOMAIN}/`,
    ...
    async interceptResponse(_request, response, body) {
  ...
  function addPreferences(url, preferences) {
    url.setQueryItem("adult", String(preferences.adult)).setQueryItem("content_rating", preferences.contentRatings.join(",")).setQueryItem("type", preferences.types);
  ...
  function fetchHome(preferences) {
    return Application.fetchJSON({ url: addPreferences(apiUrl("home"), preferences).build() });
  }
  function fetchGenres(adult) {
    return Application.fetchJSON({
      url: apiUrl("genres").setQueryItem("adult", String(adult)).build()
    });
  }
  function fetchTags(adult) {
    return Application.fetchJSON({
      url: apiUrl("tags").setQueryItem("adult", String(adult)).build()
    });
  }
  function buildSeriesUrl(options) {
    const sort = options.sort === "trending" && options.period ? `trending-${options.period}` : options.sort;
    const url = apiUrl("series").setQueryItem("sort", sort).setQueryItem("adult", String(options.adult)).setQueryItem("content_rating", options.contentRatings.join(",")).setQueryItem("limit", options.limit ?? PAGE_SIZE).setQueryItem("offset", options.offset).setQueryItem("q", options.query).setQueryItem("type", options.types).setQueryItem("genre", options.genres).setQueryItem("genre_exclude", options.excludedGenres).setQueryItem("tag", options.tags).setQueryItem("tag_exclude", options.excludedTags).setQueryItem("status", options.statuses).setQueryItem("year", options.years).setQueryItem("period", options.period);
    if (options.minChapters !== void 0) {
      url.setQueryItem("min_chapters", options.minChapters);
    }
    return url.build();
  }
  function fetchSeries(options) {
    return Application.fetchJSON({ url: buildSeriesUrl(options) });
  }
  function fetchSeriesDetails(slug) {
    return Application.fetchJSON({ url: apiUrl("series", slug).build() });
  }
  function fetchChapters(slug) {
    return Application.fetchJSON({
      url: apiUrl("series", slug, "chapters").setQueryItem("order", "asc").setQueryItem("limit", 1e5).setQueryItem("offset", 0).build()
    });
  }
  function fetchChapterDetails(slug, chapterId) {
    return Application.fetchJSON({
  ...
  // src/Chikari/parsers.ts
  function sanitizeId(value) {
    return value.replace(UNSAFE_ID, "-");
  }
  function formatCoverUrl(url, width = 400) {
  ...
  function formatRating(rating) {
    return rating === null || !isFinite(rating) ? void 0 : `\u2605 ${rating.toFixed(1)}`;
  }
  ...
  function chapterToken(number) {
    return sanitizeId(number === null ? "oneshot" : String(number));
  }
  function toContentRating(isNsfw) {
    return isNsfw ? "ADULT" /* ADULT */ : "EVERYONE" /* EVERYONE */;
  }
  function toGenreOptions(genres) {
  ...
  function toTagOptions(tags) {
    return [...new Map(tags.map((tag) => [tag.id, tag])).values()].filter((tag) => tag.count > 0).sort((left, right) => right.count - left.count || left.name.localeCompare(right.name)).slice(0, TAG_LIMIT).map((tag) => ({ id: sanitizeId(String(tag.id)), title: tag.name }));
  }
  ...
  function findHomeRow(response, slug) {
    return response.rows.find((row) => row.slug === slug)?.items ?? [];
  }
  function toFeaturedItem(series) {
    const subtitle = [
  ...
  function toRecentlyUpdatedItem(series) {
    if (series.latest_chapter === null) {
      return void 0;
    }
    const subtitle = [`Ch. ${formatChapterNumber(series.latest_chapter)}`, formatRating(series.rating) ?? ""].filter((part) => part.length > 0).join(" \u2022 ");
    const date = series.last_chapter_at !== null ? new Date(series.last_chapter_at) : void 0;
    return {
      ...
      title: series.title,
      subtitle: subtitle || void 0,
      publishDate: date !== void 0 && !isNaN(date.getTime()) ? date : void 0
    };
  }
  function subtitleFor(series) {
    return [formatSeriesType(series.type), formatRating(series.rating) ?? "", formatViews(series.views)].filter((part) => part.length > 0).join(" \u2022 ") || void 0;
  }
  ...
  function detailsToSearchResultItem(series) {
    return {
      mangaId: sanitizeId(series.slug),
      title: series.title,
      imageUrl: formatCoverUrl(series.cover_url),
  ...
  function parseMangaDetails(series) {
    const authors = series.authors.filter((credit) => credit.role === "author").map((credit) => credit.name).join(", ");
    const artists = series.authors.filter((credit) => credit.role === "artist").map((credit) => credit.name).join(", ");
    return {
      mangaId: sanitizeId(series.slug),
      mangaInfo: {
        ...
        author: authors || void 0,
        artist: artists || void 0,
        rating: series.rating === null || !isFinite(series.rating) ? void 0 : Math.min(1, Math.max(0, series.rating / 10)),
        tagGroups: [
          {
            id: "genres",
            title: "Genres",
          ...
          },
          {
            ...
            tags: series.tags.filter((tag) => !tag.is_spoiler).map((tag) => ({ id: sanitizeId(String(tag.id)), title: tag.name }))
          }
        ],
        additionalInfo: {
  ...
  function parseChapters(chapters, sourceManga) {
  ...
  function parseChapterDetails(response, chapter) {
  ...
  var Chikari = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getSettingsSections() {
      const [genres, tags] = await Promise.all([
        this.getGenreOptions().catch(() => []),
        this.getTagOptions().catch(() => [])
      ]);
      const sections = [
        {
          id: "content",
          header: "What to show",
          footer: "Titles outside these ratings and types are left out everywhere. Choosing Erotica or Pornographic also unlocks the site's adult catalogue.",
          rows: [
    ...
    async getDiscoverSections() {
      ...
      return SECTION_IDS.filter((id) => wanted === void 0 || wanted.has(id)).map(
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const chosenTypes = included.get(FILTERS.TYPE) ?? [];
      ...
      const response = await fetchSeries({
        ...
        limit: PAGE_SIZE,
        minChapters: isFinite(minChapters) && minChapters > 0 ? minChapters : void 0,
        offset,
        period: included.get(FILTERS.PERIOD)?.[0],
        query: (query.title ?? "").trim() || void 0,
        sort: included.get(FILTERS.SORT)?.[0] ?? "popular",
        statuses: included.get(FILTERS.STATUS) ?? [],
        tags: includedTags,
        types: chosenTypes.length > 0 ? chosenTypes : preferences.types,
        years: year !== void 0 ? [year] : []
      });
      const nextOffset = offset + response.items.length;
      return {
        items: response.items.map(toSearchResultItem),
        metadata: response.items.length > 0 && nextOffset < response.total ? { offset: nextOffset } : void 0
      };
    }
    ...
    get visibleSections() {
      return this.settings.stringArray(STATE_KEYS.VISIBLE_SECTIONS, new Set(SECTION_IDS));
    }
    getHome() {
  ...
  function choices(stored, allowed, fallback) {
    const kept = stored.filter((value) => allowed.includes(value));
  ...
  return __toCommonJS(Chikari_exports);
})();
