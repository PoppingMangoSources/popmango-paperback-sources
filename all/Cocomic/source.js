...
var _Sources = (() => {
  ...
  // src/Cocomic/Cocomic.ts
  var Cocomic_exports = {};
  __export(Cocomic_exports, {
    Cocomic: () => Cocomic,
    CocomicInfo: () => CocomicInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/Cocomic/models.ts
  var DOMAIN = "https://cocomic.co";
  var SECTIONS = {
    TOP_RATED: "top_rated",
  ...
  // src/Cocomic/network.ts
  var IMAGE_URL2 = /\.(avif|gif|jpe?g|jxl|png|svg|webp)([/?#]|$)/i;
  var CocomicInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
  ...
  function buildBrowseUrl(page, sortBy) {
  ...
  function buildSearchUrl(page, request) {
  ...
  function fetchBrowsePage(page, sortBy) {
    return Application.fetchDocument({ url: buildBrowseUrl(page, sortBy) });
  }
  function fetchSearchPage(page, request) {
  ...
  // src/Cocomic/parsers.ts
  var UNSAFE_ID = /[^a-zA-Z0-9._\-@()[\]%?#+=/&:]/g;
  var ORIGINAL_SCRIPT = /[\u3040-\u30ff\u3400-\u9fff\uac00-\ud7af]/;
  var ADULT_GENRES = /* @__PURE__ */ new Set(["18", "adult", "hentai", "smut"]);
  var MATURE_GENRES = /* @__PURE__ */ new Set(["mature", "soft yaoi", "soft yuri", "yaoi", "yaoibl", "yuri"]);
  var NOT_A_GENRE = /(?:publication:|status:|upload status:|read direction:|[\uf000-\uf8ff])/i;
  ...
  function imageUrlFrom(image) {
    const srcset = (image.attr("data-srcset") ?? image.attr("srcset"))?.split(",").map((entry) => {
  ...
  function contentRatingForGenres(genres, fallback = "ADULT" /* ADULT */) {
    ...
    return genres.length > 0 ? "EVERYONE" /* EVERYONE */ : fallback;
  }
  ...
  function splitTitles(value, primaryTitle) {
    return value.split(/\s*[;/]\s*/).map(cleanText).filter(
      (title, index, values) => title.length > 0 && !/^none$/i.test(title) && title.toLowerCase() !== primaryTitle.toLowerCase() && values.indexOf(title) === index
    );
  }
  ...
  function chapterTitle(value) {
    const remainder = cleanText(
      value.replace(/\bvol(?:ume)?\.?\s*[-_:]?\s*\d+(?:\.\d+)?/i, "").replace(/\b(?:chapter|chap|ch\.?|episode|ep\.?)\s*[-_:]?\s*\d+(?:\.\d+)?/i, "").replace(/^[-:.\s]+|[-:.\s]+$/g, "")
    );
  ...
  function parseListingChapter($, container) {
    ...
    const dateContainer = chapterRow.length > 0 ? chapterRow.find(".post-on") : container.find(".meta-item.post-on").first();
    return {
      chapterId,
      title,
  ...
  function parseMangaList($, elements = $(".c-tabs-item__content, .page-item-detail, .related__item, .slider__item").toArray()) {
    const items = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of elements) {
      const item = $(element);
      const titleLink = item.find(
      ...
      seen.add(mangaId);
      const genres = item.find(".mg_genres .summary-content a, .genres-content a").map((_, link) => cleanText($(link).text())).toArray().filter((genre) => genre.length > 0);
  ...
  function parseHomepageRail($, title) {
    const heading = $(".tp-heading").filter((_, element) => {
  ...
  function parseGenreTags($) {
    ...
    for (const element of $('input[name="genre[]"]').toArray()) {
      ...
      if (id.length === 0 || title.length === 0 || title.length > 64 || NOT_A_GENRE.test(title) || seenIds.has(id) || seenTitles.has(title.toLowerCase())) {
        continue;
      }
      seenIds.add(id);
  ...
  function parseChapterDetails($, chapter) {
    const pages = $(".reading-content .page-break img, .reading-content img.wp-manga-chapter-img").toArray().map((element) => imageUrlFrom($(element))).filter((url) => url.length > 0);
    if (pages.length === 0) {
      throw new Error(
  ...
  var Cocomic = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.TOP_RATED, title: "Top Rated", type: "featured" /* featured */ },
        { id: SECTIONS.ONLY_COCOMIC, title: "Only Cocomic", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.NEW_RELEASES, title: "New Releases", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST_UPDATES, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.TODAYS_OFFICIAL, title: "Today's Official", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.YAOI, title: "Yaoi", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.MANHWA, title: "Manhwa", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.SMUT, title: "Smut", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
     ...
     */
    async paged(metadata, fetch, map) {
  ...
  return __toCommonJS(Cocomic_exports);
})();
