...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/KingOfShojo/models.ts
  var DEFAULT_DOMAIN = "https://kingofshojo.com";
  var MANGA_DIR = "manga";
  var NEXT_PAGE_SELECTOR = "div.pagination .next, div.hpage .r, a:has(img[alt=Next])";
  var BASE_URL_KEY = "kingofshojo.baseUrlOverride";
  var IMAGE_MODE_KEY = "kingofshojo.imageMode";
  var SETTINGS_KEYS = [BASE_URL_KEY, IMAGE_MODE_KEY];
  var IMAGE_MODES = ["saver", "quality", "original"];
  var IMAGE_MODE_DEFAULT = "saver";
  var IMAGE_MODE_OPTIONS = [
  ...
  var ADULT_GENRE_NAMES = /* @__PURE__ */ new Set([
    "adult",
    "adult content",
    "smut",
    "hentai",
  ...
  // src/KingOfShojo/network.ts
  var IMAGE_URL2 = /\.(avif|gif|jpe?g|jxl|png|webp)(\?|$)/i;
  var USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1";
  function withHeaders(headers, overrides) {
    const result = {};
  ...
  var KingOfShojoInterceptor = class extends PaperbackInterceptor {
    ...
    async interceptRequest(request) {
      ...
      if (IMAGE_URL2.test(request.url)) {
        return {
          ...request,
  ...
  // src/KingOfShojo/parsers.ts
  var CARD_SELECTOR = ".utao .uta .imgu, .listupd .bs .bsx, .listo .bs .bsx, .bsx";
  var DETAILS_SCOPE = "div.bigcontent, div.animefull, div.main-info, div.postbody";
  var TITLE_SELECTOR = "h1.entry-title, .ts-breadcrumb li:last-child span";
  var THUMB_SELECTOR = ".infomanga > div[itemprop=image] img, .thumb img";
  var DESC_SELECTOR = ".desc, .entry-content[itemprop=description]";
  var ALT_NAME_SELECTOR = ".alternative, .wd-full:contains(alt) span, .alter, .seriestualt";
  var GENRE_SELECTOR = "div.gnr a, .mgen a, .seriestugenre a";
  var AUTHOR_SELECTOR = ".infotable tr:contains(Author) td:last-child, .tsinfo .imptdt:contains(Author) i, .fmed b:contains(Author)+span";
  var ARTIST_SELECTOR = ".infotable tr:contains(Artist) td:last-child, .tsinfo .imptdt:contains(Artist) i, .fmed b:contains(Artist)+span";
  var STATUS_SELECTOR = ".infotable tr:contains(Status) td:last-child, .tsinfo .imptdt:contains(Status) i, .fmed b:contains(Status)+span";
  var CHAPTER_SELECTOR = "div.bxcl li, div.cl li, #chapterlist li, ul li:has(div.chbox):has(div.eph-num)";
  var CHAPTER_NAME_SELECTOR = ".lch a, .chapternum";
  var CHAPTER_DATE_SELECTOR = ".chapterdate";
  var PAGE_SELECTOR = "div#readerarea img";
  var IMAGE_LIST = /"images"\s*:\s*(\[[\s\S]*?\])/;
  var GENRE_FILTER_SELECTOR = "ul.genrez li";
  var MONTHS = {
    jan: 0,
    january: 0,
    feb: 1,
  ...
  function parseMangaId(href) {
    const cleaned = href.replace(/[?#].*$/, "").replace(/\/+$/, "");
    const marker = `/${MANGA_DIR}/`;
    const index = cleaned.indexOf(marker);
    const slug = index !== -1 ? cleaned.slice(index + marker.length).split("/")[0] ?? "" : cleaned.split("/").pop() ?? "";
    return toSafeId(slug);
  }
  function parseChapterId(href) {
    const cleaned = href.replace(/^(?:https?:)?\/\/[^/]+/i, "").replace(/[?#].*$/, "").replace(/^\/+|\/+$/g, "");
  ...
  function imgAttr(base, img) {
    ...
    if (src.length === 0) {
      ...
      if (srcset !== void 0) {
        const best = srcset.split(",").map((part) => part.trim().split(/\s+/)).map(([url, width]) => ({
          url: url ?? "",
  ...
  function cardTitle(unit, link) {
    const img = unit.find("img").first();
    const raw = unit.find(".bigor .tt a, .tt").first().text().trim() || img.attr("title") || link.attr("title") || link.text();
    return Application.decodeHTMLEntities((raw || "").trim());
  }
  function parseCard($, base, element) {
    const unit = $(element);
    ...
    return {
      ...
      subtitle: rating.length > 0 ? `\u2605 ${rating}` : chapter || void 0,
  ...
  function dedupeCards(cards) {
    ...
    for (const card of cards) {
      if (card !== void 0 && !seen.has(card.mangaId)) {
  ...
  function widgetByHeading($, heading) {
    return $(`.releases:contains("${heading}")`).first().closest(".bixbox, .section");
  }
  function parseWidgetCards($, base, heading) {
    return dedupeCards(
      widgetByHeading($, heading).find(".bsx").toArray().map((element) => parseCard($, base, element))
    );
  }
  function parsePopularSeries($, base, rangeClass) {
    const scope = widgetByHeading($, "Popular Series");
    const cards = [];
    const seen = /* @__PURE__ */ new Set();
    for (const element of scope.find(`.serieslist.${rangeClass} ul li, #wpop-items .${rangeClass} ul li`).toArray()) {
      const row = $(element);
      ...
      const isAdult = row.find('a[href*="/genres/"]').toArray().some((genre) => ADULT_GENRE_NAMES.has($(genre).text().trim().toLowerCase()));
      seen.add(mangaId);
      ...
      cards.push({
        ...
        subtitle: rating.length > 0 ? `\u2605 ${rating}` : void 0,
        rating: rating || void 0,
        isAdult
      });
    }
  ...
  function parseLatestUpdate($, base) {
    ...
    for (const element of scope.find(".bsx").toArray()) {
      ...
      cards.push({
        ...
        chapterId: chapterHref.length > 0 ? parseChapterId(chapterHref) : void 0,
        chapterName: chapterName || void 0,
        publishDate: timeText.length > 0 ? parseSiteDate(timeText) : void 0
      });
    }
    return cards;
  }
  function parseGenreFilter($) {
    const genres = [];
    const seen = /* @__PURE__ */ new Set();
  ...
  function collectText($, scope, selector) {
    const out = [];
  ...
  function parseMangaDetails($, base, mangaId, contentRating) {
    ...
    const genreTags = [];
    const seenGenre = /* @__PURE__ */ new Set();
    ...
    const tagGroups = genreTags.length > 0 ? [{ id: "genres", title: "Genres", tags: genreTags }] : [];
    const effectiveRating = genreTags.some((tag) => ADULT_GENRE_NAMES.has(tag.title.trim().toLowerCase())) ? "ADULT" /* ADULT */ : contentRating;
    return {
      mangaId,
      mangaInfo: {
        primaryTitle,
        ...
        thumbnailUrl,
        synopsis,
        author: collectText($, scope, AUTHOR_SELECTOR).join(", ") || void 0,
        artist: collectText($, scope, ARTIST_SELECTOR).join(", ") || void 0,
  ...
  function parseStatus(status) {
    ...
    if (text.includes("ongoing") || text.includes("on going") || text.includes("publishing") || text.includes("updating")) {
      return "Ongoing";
    }
  ...
  function parseChapters($, sourceManga) {
    ...
    for (const element of $(CHAPTER_SELECTOR).toArray()) {
      ...
      chapters.push({
        ...
        sourceManga,
        title,
        chapNum: parseChapterNumber2(title),
        volume: 0,
  ...
  function parseChapterPages($, base) {
    const pages = [];
    ...
    if (pages.length === 0) {
      ...
      if (match !== null) {
        try {
          for (const entry of JSON.parse(match[1] ?? "[]")) {
  ...
  function proxyImage(url, mode) {
    if (mode === "original") {
      return url;
    }
    const match = /^https?:\/\/(.+)$/i.exec(url);
    if (match === null || /\/\/wsrv\.nl\//i.test(url) || /[?#]/.test(url) || !/\.(avif|gif|jpe?g|jxl|png|webp)$/i.test(url)) {
      return url;
    }
  ...
  function parseSiteDate(text) {
    ...
    if (relative !== null) {
      ...
      const ms = unit === "second" ? 1e3 : unit === "min" ? 6e4 : unit === "hour" ? 36e5 : unit === "day" ? 864e5 : unit === "week" ? 6048e5 : unit === "month" ? 2592e6 : 31536e6;
  ...
  // src/KingOfShojo/KingOfShojo.ts
  var KingOfShojoInfo = sourceInfo({
    name: "King of Shojo",
  ...
  var KingOfShojo = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
    ...
    getSettingsSections() {
      return [
        ...
        },
        {
          id: "images",
          header: "Images",
          footer: "Reader pages are served full size, at well over a megabyte each. The two compressed settings pass them through an image proxy so chapters open quickly; Original fetches them straight from the site, which can be slow.",
          rows: [
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.POPULAR_TODAY, title: "Popular Today", type: "featured" /* featured */ },
        { id: SECTIONS.RECOMMENDATION, title: "Recommendation", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST_UPDATE, title: "Latest Update", type: "chapterUpdates" /* chapterUpdates */ }
      ];
    }
    async getDiscoverSectionItems(section) {
      ...
      switch (section.id) {
        case SECTIONS.POPULAR_TODAY:
          return {
            items: parseWidgetCards($, this.baseUrl, HOME_HEADINGS.POPULAR_TODAY).map((card) => ({
              ...
              subtitle: card.rating !== void 0 ? `\u2605 ${card.rating}` : void 0
            }))
          };
        ...
        case SECTIONS.LATEST_UPDATE:
          return {
            items: parseLatestUpdate($, this.baseUrl).flatMap(
              (card) => card.chapterId === void 0 ? [] : [
                {
    ...
    async getFilterSections() {
      return [
        ...
        },
        {
          id: FILTERS.GENRE,
          title: "Genres",
          tags: (await this.getGenres()).map(
            (genre) => filterTag(FILTERS.GENRE, genre.id, genre.value)
          )
        },
        {
          id: FILTERS.STATUS,
    ...
    async getSearchResultItems(query, metadata) {
      ...
      if (range !== void 0 && title.length === 0) {
        return {
          items: parsePopularSeries(await this.getHomePage(), this.baseUrl, range).map((card) => ({
            ...
            imageUrl: card.imageUrl,
            subtitle: card.subtitle,
            contentRating: card.isAdult === true ? "ADULT" /* ADULT */ : "MATURE" /* MATURE */
          }))
        };
      }
      ...
      const url = URL(this.baseUrl).addPathComponent(`${MANGA_DIR}/`).setQueryItem("title", title).setQueryItem("page", page).setQueryItem("order", included.get(FILTERS.SORT)?.[0]).setQueryItem("status", included.get(FILTERS.STATUS)?.[0]).setQueryItem("type", included.get(FILTERS.TYPE)?.[0]);
      ...
      return {
        ...
        metadata: $(NEXT_PAGE_SELECTOR).length > 0 ? { page: page + 1 } : void 0
      };
    }
    async getMangaInfo(mangaId) {
      const $ = await fetchPage(this.mangaUrl(mangaId));
      return parseMangaDetails($, this.baseUrl, mangaId, "MATURE" /* MATURE */);
    }
    ...
    async getPages(chapter) {
      ...
      const pages = parseChapterPages(await fetchPage(url), this.baseUrl).map(
        (page) => proxyImage(page, mode)
      );
      if (pages.length === 0) {
