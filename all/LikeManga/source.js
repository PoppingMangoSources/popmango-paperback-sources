...
var _Sources = (() => {
  ...
  // src/LikeManga/LikeManga.ts
  var LikeManga_exports = {};
  __export(LikeManga_exports, {
    LikeManga: () => LikeManga,
    LikeMangaInfo: () => LikeMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/LikeManga/models.ts
  var DOMAIN = "https://likemanga.ink";
  var SECTIONS = {
  ...
  var TOP_SERIES_SORT = "top-month";

  // src/LikeManga/network.ts
  ...
  var LikeMangaInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
    ...
    async interceptResponse(request, response, body) {
      ...
      if (headerValue(response.headers, "cf-mitigated") === "challenge" || blocked && /(?:Just a moment|cf-chl-|_cf_chl_opt)/i.test(text)) {
  ...
  function fetchHotPage(page = 1) {
    const url = page === 1 ? `${DOMAIN}/hot/` : URL(DOMAIN).setQueryItem("act", "search").setQueryItem("f[status]", "all").setQueryItem("f[sortby]", "hot").setQueryItem("pageNum", page).build();
    return Application.fetchDocument({ url });
  }
  function fetchAdvancedSearchPage() {
    return Application.fetchDocument({ url: `${DOMAIN}/searchadvance/` });
  }
  function fetchSearchPage(request) {
    return Application.fetchDocument({ url: buildSearchUrl(request) });
  }
  function fetchContentPage(id) {
    return Application.fetchDocument({ url: URL(DOMAIN).setPath(decodePathId(id)).build() });
  }
  async function fetchChapterListPage(mangaNumericId, page) {
    const response = await Application.fetchJSON({
      url: URL(DOMAIN).setQueryItem("act", "ajax").setQueryItem("code", "load_list_chapter").setQueryItem("manga_id", mangaNumericId).setQueryItem("page_num", page).setQueryItem("chap_id", "0").setQueryItem("keyword", "").build()
    });
  ...
  // src/LikeManga/parsers.ts
  var ADULT_GENRES = /* @__PURE__ */ new Set(["adult", "hentai", "smut"]);
  var MATURE_GENRES = /* @__PURE__ */ new Set(["ecchi", "mature", "yaoi", "yuri"]);
  var KOREAN_TITLE = /[ᄀ-ᇿ㄰-㆏가-힯][ᄀ-ᇿ㄰-㆏가-힯0-9\s·-]*/g;
  ...
  function normalisedPath(href) {
    const path = Application.decodeHTMLEntities(href).replace(/^https?:\/\/(?:www\.)?likemanga\.ink\//i, "").replace(/[?#].*$/, "").replace(/^\/+|\/+$/g, "");
    try {
  ...
  function imageUrlFrom(image) {
    ...
    return toAbsoluteUrl(
      image.attr("data-cfsrc") ?? image.attr("data-src") ?? image.attr("data-lazy-src") ?? srcset ?? image.attr("src")
    );
  }
  function koreanTitleFrom(value) {
    return (cleanText(value).match(KOREAN_TITLE) ?? []).map(cleanText).sort((left, right) => right.length - left.length)[0];
  }
  function labelledValue($, container, label) {
  ...
  function formatChapterLabel(title) {
  ...
  function chapterTitleFromLink(link) {
  ...
  function parseListingChapter($, element) {
    const row = $(element);
  ...
  function parseMangaList($) {
    ...
    for (const element of $(".video").toArray()) {
      ...
      const tooltip = tooltipSelector !== void 0 ? $(tooltipSelector) : card.find("[data-missing-tooltip]");
      const genres = labelledValue($, tooltip, "Genres").split(",").map(cleanText).filter((genre) => genre.length > 0);
      ...
      items.push({
        ...
        genres,
        status: labelledValue($, tooltip, "Status") || void 0,
        views: labelledValue($, tooltip, "View") || void 0,
        comments: labelledValue($, tooltip, "Comment") || void 0,
        follows: labelledValue($, tooltip, "Follow") || void 0,
        rating: Number.isFinite(rawRating) ? rawRating : void 0,
        updatedDate: parseListingDate(labelledValue($, tooltip, "Updated")),
        chapters: card.find(".list-group-item").toArray().flatMap((chapter) => {
  ...
  function parseNewManga($) {
    ...
    for (const element of $(".items-slide .item").toArray()) {
      ...
      items.push({
        ...
        chapter: chapterHref.length > 0 && chapterTitle.length > 0 ? {
  ...
  function parseMangaDetails($, mangaId) {
    ...
    const genres = $(".list-info .kind a").map((_, element) => cleanText($(element).text())).toArray().filter((genre) => genre.length > 0);
    const tags = $(".list-info .kind a").map((_, element) => {
      const link = $(element);
      const href = link.attr("href")?.replace(/\/+$/, "") ?? "";
      return { id: href.split("/").pop() ?? "", title: cleanText(link.text()) };
    }).toArray().filter((tag) => tag.id.length > 0 && tag.title.length > 0);
  ...
  function parseChapterPageInfo($) {
    ...
    return {
      mangaNumericId: $("#title-detail-manga").attr("data-manga") || void 0,
      lastPage
    };
  }
  function parseChapters($, fragments, sourceManga) {
    ...
    return entries.map((entry, index) => ({
      ...
      title: formatChapterTitle(entry.name) || void 0,
      volume: 0,
      // The list runs newest first; the app wants oldest lowest.
      sortingIndex: entries.length - index,
      publishDate: parseListingDate(entry.dateText) ?? (/^new$/i.test(entry.dateText) ? fallbackDate : void 0)
    }));
  }
  function parseChapterPages($, chapter) {
    const pages = [];
    const token = $("#next_img_token").attr("value") ?? "";
    const cdnUrl = $("#currentlink").attr("value")?.replace(/\/+$/, "") ?? "";
    const encodedPayload = token.split(".")[1];
    if (encodedPayload !== void 0 && encodedPayload.length > 0 && cdnUrl.length > 0) {
      try {
        const payload = JSON.parse(Application.base64Decode(encodedPayload));
        if (typeof payload.data === "string") {
          const manifest = JSON.parse(Application.base64Decode(payload.data));
  ...
  function hasNextPage($) {
    return $(".pagination a").toArray().some((element) => cleanText($(element).text()) === "\xBB");
  }
  function toDiscoverItem(item, detail) {
    return {
      mangaId: item.mangaId,
      title: item.title,
      imageUrl: item.imageUrl,
      subtitle: joinDetails([
        item.chapters[0] === void 0 ? void 0 : formatChapterLabel(item.chapters[0].title),
        detail
      ])
    };
  }
  function toFollowedItem(item) {
    return toDiscoverItem(item, item.follows === void 0 ? void 0 : `\u2661 ${item.follows}`);
  }
  function toHotItem(item) {
    return toDiscoverItem(item, item.views === void 0 ? void 0 : `${item.views} views`);
  }
  function toNewMangaItem(item) {
    return {
      mangaId: item.mangaId,
      title: item.title,
      imageUrl: item.imageUrl,
      subtitle: joinDetails([
        item.chapter === void 0 ? void 0 : formatChapterLabel(item.chapter.title),
        "NEW"
      ])
    };
  }
  function toLatestReleaseItem(item) {
    ...
    return {
      ...
      publishDate: parseListingDate(chapter.dateText) ?? (chapter.isNew ? item.updatedDate : void 0)
    };
  }
  function toSearchResultItem(item) {
    return {
      ...
      imageUrl: item.imageUrl,
      subtitle: joinDetails([
        item.views === void 0 ? void 0 : `${item.views} views`,
        item.chapters[0] === void 0 ? void 0 : formatChapterLabel(item.chapters[0].title)
      ]),
  ...
  var LikeMangaInfo = sourceInfo({
    ...
    capabilities: ["chapters" /* CHAPTERS */, "homePage" /* HOME_PAGE */, "cloudflare" /* CLOUDFLARE */]
  });
  var LikeManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.MOST_FOLLOWED, title: "Most Followed", type: "featured" /* featured */ },
        { id: SECTIONS.NEW_MANGA, title: "New Manga", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST_RELEASES, title: "Latest Releases", type: "chapterUpdates" /* chapterUpdates */ },
        ...
        { id: SECTIONS.TOP_SERIES, title: "Top Series This Month", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.HOT, title: "Hot", type: "prominentCarousel" /* prominentCarousel */ }
      ];
    }
      ...
      switch (section.id) {
        case SECTIONS.MOST_FOLLOWED:
          return this.listingSection(
            "follow",
            metadata,
            (item) => item.description === void 0 ? void 0 : toFollowedItem(item)
          );
        ...
        case SECTIONS.HOT: {
          ...
          return {
            ...
            metadata: hasNextPage(document) ? { page: page + 1 } : void 0
          };
        }
        default:
          return { items: [] };
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const request = {
        ...
        status: (included.get(FILTERS.STATUS) ?? []).length === 1 && includedStatuses.length === 0 ? included.get(FILTERS.STATUS)?.[0] : void 0,
    ...
    async getChapterList(sourceManga) {
      ...
      const fragments = numericId !== void 0 && pageInfo.lastPage > 1 ? await Promise.all(
        Array.from(
          { length: pageInfo.lastPage - 1 },
          (_, index) => fetchChapterListPage(numericId, index + 2)
        )
      ) : [];
    ...
    async listingSection(sortBy, metadata, map) {
      const page = metadata?.page ?? 1;
     ...
     */
    matchesFilters(item, included, excluded) {
  ...
  function normaliseFilterValue(value) {
    return value.replace(/%20/gi, " ").trim().toLowerCase().replace(/[\s_]+/g, "-");
  }
  function statusFilterId(status) {
  ...
  return __toCommonJS(LikeManga_exports);
})();
