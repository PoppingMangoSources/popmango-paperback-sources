...
var _Sources = (() => {
  ...
  // src/MangaBerri/MangaBerri.ts
  var MangaBerri_exports = {};
  __export(MangaBerri_exports, {
    MangaBerri: () => MangaBerri,
    MangaBerriInfo: () => MangaBerriInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/MangaBerri/models.ts
  var DOMAIN = "https://mangaberri.com";
  var BASE_URL_KEY = "mangaberri.baseUrl";
  var SETTINGS_KEYS = [BASE_URL_KEY];
  var SECTIONS = {
  ...
  var MATURE_GENRES = ["seinen", "ecchi", "harem", "mature", "adult", "smut"];

  // src/MangaBerri/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif)([?#]|$)/i;
  var userAgent;
  function getUserAgent() {
    return userAgent ??= Application.getDefaultUserAgent();
  }
  var MangaBerriInterceptor = class extends PaperbackInterceptor {
    ...
    async interceptRequest(request) {
      ...
      return {
        ...
        headers: {
          ...
          accept: isImage ? "image/avif,image/webp,image/apng,image/png,image/*,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
        }
      };
    }
  };
  ...
  function genreUrl(baseUrl, name) {
    return `${baseUrl}/genre.php?genre=${encodeURIComponent(name).replace(/%2F/gi, "/")}`;
  }

  // src/MangaBerri/parsers.ts
  ...
  function ratingFrom(text) {
    ...
    return value !== void 0 && Number.parseFloat(value) > 0 ? value : void 0;
  }
  ...
  function parseRelativeDate(value) {
    ...
    return offset > 0 ? new Date(Date.now() - offset) : void 0;
  }
  function contentRatingForGenres(genres) {
    const normalised = genres.map((genre) => genre.trim().toLowerCase());
    return normalised.some((genre) => MATURE_GENRES.includes(genre)) ? "MATURE" /* MATURE */ : "EVERYONE" /* EVERYONE */;
  }
  function coverFrom(baseUrl, element) {
    const image = element.find("img").first();
    return absoluteUrl(baseUrl, image.attr("data-src") ?? image.attr("src") ?? "");
  }
  function parseCard($, baseUrl, item) {
    const links = item.find("a[href]").toArray().map((element) => $(element));
    ...
    const chapterLabel2 = chapterLink === void 0 ? "" : cleanText(chapterLink.text());
    return {
      slug,
      title,
      ...
      chapterLabel: chapterLabel2.length > 0 ? chapterLabel2 : void 0,
      genres: []
    };
  }
  function sectionContainer($, title) {
    return $(".section-title").filter((_, element) => cleanText($(element).text()) === title).first().closest(".section-container");
  }
  function parseCarouselSection($, baseUrl, title) {
    return sectionContainer($, title).find(".manga-item").toArray().flatMap((element) => {
      const card = parseCard($, baseUrl, $(element));
      return card === void 0 ? [] : [card];
    });
  }
  function parseLatestSection($, baseUrl) {
    return sectionContainer($, HOME_TITLES.LATEST).find(".manga-horizontal-item").toArray().flatMap((element) => {
      const item = $(element);
      const links = item.find("a[href]").toArray().map((anchor) => $(anchor));
      ...
      const chapterLabel2 = cleanText(chapterLink.text());
      ...
      return [
        {
          ...
          chapterId: chapterIdFromHref(chapterLink.attr("href")),
          chapterLabel: chapterLabel2.length > 0 ? chapterLabel2 : void 0,
          updatedAt: updatedAt.length > 0 ? updatedAt : void 0,
          genres: item.find('a[href*="genre.php"]').toArray().map((genre) => cleanText($(genre).text())).filter((genre) => genre.length > 0)
        }
      ];
    });
  }
  function parseListingCards(html, baseUrl) {
    const $ = Application.loadDocument(html);
    return $(".manga-item").toArray().flatMap((element) => {
      const card = parseCard($, baseUrl, $(element));
      return card === void 0 ? [] : [card];
    });
  }
  ...
  function chapterLabel(card) {
    if (card.chapterLabel === void 0) {
      return void 0;
    }
    const number = chapterNumberFrom(card.chapterLabel);
    return number === void 0 ? card.chapterLabel : `Ch. ${number}`;
  }
  function toDiscoverItem(card) {
    return {
      ...
      title: card.title,
      imageUrl: card.cover,
      subtitle: joinDetails([chapterLabel(card), card.rating === void 0 ? void 0 : `\u2605 ${card.rating}`])
    };
  }
  function toRankedItem(card, index) {
    return {
      mangaId: card.slug,
      title: card.title,
      imageUrl: card.cover,
  ...
  function toLatestItem(card) {
    ...
    return {
      ...
      publishDate: card.updatedAt === void 0 ? void 0 : parseRelativeDate(card.updatedAt)
    };
  }
  function toSearchResultItem(card) {
    return {
      ...
      title: card.title,
      imageUrl: card.cover,
      subtitle: card.rating === void 0 ? void 0 : `\u2605 ${card.rating}`,
      contentRating: contentRatingForGenres(card.genres)
    };
  }
  function detailField($, label) {
    return cleanText(
      $(".section-status.row, .comic-attrs .column").toArray().map((element) => $(element)).find((row) => cleanText(row.children().first().text()) === label)?.children().last().text() ?? ""
    );
  }
  function parseMangaDetails(html, baseUrl, mangaId) {
    const $ = Application.loadDocument(html);
    ...
    const secondaryTitles = cleanText($(".comic-info-container h2").first().text()).split(";").map((alias) => alias.trim()).filter((alias) => alias.length > 0 && alias.toLowerCase() !== title.toLowerCase());
    const seen = /* @__PURE__ */ new Set();
    const tags = $('.comic-attrs a[href*="genre.php?genre="]').toArray().flatMap((element) => {
    ...
    return {
      ...
      mangaInfo: {
        ...
        rating: ratingText === void 0 ? void 0 : Math.min(1, Math.max(0, Number.parseFloat(ratingText) / 5)),
  ...
  function parseChapterList(html, sourceManga) {
    const $ = Application.loadDocument(html);
    const entries = $(".chapters-container a[href]").toArray().flatMap((element) => {
      const chapterId = chapterIdFromHref($(element).attr("href"));
      return chapterId === void 0 ? [] : [{ chapterId, label: cleanText($(element).text()) }];
    });
    const seen = /* @__PURE__ */ new Set();
  ...
  function parseReaderPages(html, baseUrl, chapter) {
    const $ = Application.loadDocument(html);
    const pages = $(".reading-container img").toArray().map((element) => absoluteUrl(baseUrl, $(element).attr("data-src") ?? $(element).attr("src") ?? "")).filter((url) => url.length > 0);
    if (pages.length === 0) {
  ...
  var MangaBerriInfo = sourceInfo({
    ...
    capabilities: ["chapters" /* CHAPTERS */, "homePage" /* HOME_PAGE */, "settings" /* SETTINGS */]
  });
  var MangaBerri = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.MOST_VIEWED, title: "Most Viewed", type: "featured" /* featured */ },
        { id: SECTIONS.WEEKLY, title: "Top Weekly", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.SHOUNEN, title: "Top Shounen", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST, title: "Latest Update", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.SEINEN, title: "Top Seinen", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.POPULAR_TODAY, title: "Popular Today", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.MANHWA_MANHUA, title: "Top Manhwa/Manhua", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const url = term.length > 0 ? `${this.baseUrl}/search.php?keyword=${encodeURIComponent(term)}` : genreId !== void 0 ? genreUrl(this.baseUrl, genreName(genreId)) : `${this.baseUrl}/home.php`;
      ...
      return {
        ...
        metadata: fresh.length > 0 ? { page: page + 1, seen: [...seen] } : void 0
      };
    }
    ...
    getHomePage() {
      return this.homePage ??= fetchHtml(`${this.baseUrl}/home.php`).then(
        (html) => Application.loadDocument(html)
      );
    }
    ...
    async resolvePastedUrl(title) {
      ...
      if (slug === void 0 || slug.endsWith(".php")) {
        return void 0;
      }
  ...
  return __toCommonJS(MangaBerri_exports);
})();
