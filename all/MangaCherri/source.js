...
var _Sources = (() => {
  ...
  // src/MangaCherri/MangaCherri.ts
  var MangaCherri_exports = {};
  __export(MangaCherri_exports, {
    MangaCherri: () => MangaCherri,
    MangaCherriInfo: () => MangaCherriInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/MangaCherri/models.ts
  var DOMAIN = "https://mangacherri.com";
  var BASE_URL_KEY = "mangacherri.baseUrl";
  ...
  var MATURE_GENRES = ["josei", "seinen", "mature", "adult", "smut", "ecchi"];

  // src/MangaCherri/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif)([?#]|$)/i;
  var userAgent;
  function getUserAgent() {
    return userAgent ??= Application.getDefaultUserAgent();
  }
  var MangaCherriInterceptor = class extends PaperbackInterceptor {
  ...
  function parseCarouselCard(baseUrl, item) {
  ...
  function homeSectionCards($, baseUrl, title, itemSelector, parse) {
    const container = $(".section-title").filter((_, element) => cleanText($(element).text()) === title).first().closest(".section-container");
    return container.find(itemSelector).toArray().flatMap((element) => {
  ...
  function parseListingCard($, baseUrl, item) {
  ...
  function toDiscoverItem(card) {
    return {
      ...
      subtitle: joinDetails([
        ...
        card.views === void 0 ? void 0 : `${card.views} views`
      ])
    };
  }
  function toWeeklyItem(card) {
    return {
      mangaId: card.slug,
      title: card.title,
      imageUrl: card.cover,
  ...
  // src/MangaCherri/MangaCherri.ts
  var MangaCherriInfo = sourceInfo({
    name: "MangaCherri",
  ...
  var MangaCherri = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: SECTIONS.WEEKLY, title: "Weekly", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST, title: "Latest Chapter", type: "chapterUpdates" /* chapterUpdates */ },
        { id: SECTIONS.POPULAR_NOW, title: "Popular Now", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.COMPLETED, title: "Completed Romance", type: "simpleCarousel" /* simpleCarousel */ }
      ];
    }
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const url = term.length > 0 ? `${this.baseUrl}/search.php?keyword=${encodeURIComponent(term)}` : genreId !== void 0 ? `${this.baseUrl}/genre.php?genre=${encodeURIComponent(genreName(genreId))}` : `${this.baseUrl}/home.php`;
  ...
  return __toCommonJS(MangaCherri_exports);
})();
