...
var _Sources = (() => {
  ...
  // src/MangaTown/MangaTown.ts
  var MangaTown_exports = {};
  __export(MangaTown_exports, {
    MangaTown: () => MangaTown,
    MangaTownInfo: () => MangaTownInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/MangaTown/models.ts
  var DOMAIN = "https://www.mangatown.com";
  var READER_CONCURRENCY = 4;
  var VISIBLE_SECTIONS_KEY = "mangatown.visibleSections";
  var SETTINGS_KEYS = [VISIBLE_SECTIONS_KEY];
  var SECTIONS = {
    FEATURED: "featured",
    HOT: "hot",
    LATEST: "latest",
  ...
  var SECTION_DEFINITIONS = {
    [SECTIONS.FEATURED]: { id: SECTIONS.FEATURED, title: "Featured Manga", type: "featured" /* featured */ },
    [SECTIONS.HOT]: { id: SECTIONS.HOT, title: "Hot Manga", type: "prominentCarousel" /* prominentCarousel */ },
    [SECTIONS.LATEST]: { id: SECTIONS.LATEST, title: "Latest Updates", type: "chapterUpdates" /* chapterUpdates */ },
    [SECTIONS.NEW]: { id: SECTIONS.NEW, title: "New Manga Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.ROMANCE]: { id: SECTIONS.ROMANCE, title: "Romance Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.SHOUNEN]: { id: SECTIONS.SHOUNEN, title: "Shounen Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.TOP_SHOUNEN]: { id: SECTIONS.TOP_SHOUNEN, title: "Top Shounen", type: "featured" /* featured */ },
    [SECTIONS.SEINEN]: { id: SECTIONS.SEINEN, title: "Seinen Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.TOP_SEINEN]: { id: SECTIONS.TOP_SEINEN, title: "Top Seinen", type: "featured" /* featured */ },
    [SECTIONS.SHOUJO]: { id: SECTIONS.SHOUJO, title: "Shoujo Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.TOP_SHOUJO]: { id: SECTIONS.TOP_SHOUJO, title: "Top Shoujo", type: "featured" /* featured */ },
    [SECTIONS.YAOI]: { id: SECTIONS.YAOI, title: "Yaoi Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.SHOUNEN_AI]: { id: SECTIONS.SHOUNEN_AI, title: "Shounen Ai Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.JOSEI]: { id: SECTIONS.JOSEI, title: "Josei Releases", type: "simpleCarousel" /* simpleCarousel */ },
    [SECTIONS.TOP_YAOI]: { id: SECTIONS.TOP_YAOI, title: "Top Yaoi", type: "featured" /* featured */ }
  };
  var SECTION_ORDER = Object.keys(SECTION_DEFINITIONS);
  var SECTION_OPTIONS = SECTION_ORDER.map((id) => ({
    id,
  ...
  var GENRE_TITLES = new Map(
    [...GENRES, ...DEMOGRAPHICS].map((tag) => [tag.id, tag.title])
  );

  // src/MangaTown/network.ts
  var IMAGE_URL2 = /\.(avif|gif|jpe?g|jxl|png|svg|webp)([/?#]|$)/i;
  var MangaTownInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      ...
      return {
        ...
        headers: {
          ...
          accept: request.headers.accept ?? (isImage ? "image/avif,image/webp,image/png,image/jpeg,image/*,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8"),
  ...
  function sortedUrl(url, token) {
    return token !== void 0 && token.length > 0 ? `${url}?${token}` : url;
  }
  function directoryUrl(page, filters = {}) {
  ...
  function hotUrl(page, demographic, sortToken) {
    const segment = demographic !== void 0 ? `${demographic}/` : "";
    return sortedUrl(`${DOMAIN}/hot/${segment}${page}.htm`, sortToken);
  }
  function searchUrl(page, request) {
  ...
  function chapterUrl(mangaId, chapterId) {
    return `${DOMAIN}/manga/${mangaId}/${chapterId}/`;
  }
  function fetchListingPage(url) {
    return Application.fetchDocument({ url });
  }
  function fetchFeaturedPage() {
    return Application.fetchDocument({ url: `${DOMAIN}/featured/` });
  }
  function fetchMangaPage(mangaId) {
    return Application.fetchDocument({ url: mangaUrl(mangaId) });
  }
  function fetchChapterPage(url) {
    return Application.fetchDocument({ url });
  }

  // src/MangaTown/parsers.ts
  var UNSAFE_ID = /[^a-zA-Z0-9._\-@()[\]%?#+=/&:]/g;
  var ADULT_GENRES = /* @__PURE__ */ new Set(["adult", "hentai", "lolicon", "shotacon"]);
  var MATURE_GENRES = /* @__PURE__ */ new Set(["ecchi", "mature", "smut", "yaoi", "yuri"]);
  var MONTHS = {
    jan: 0,
    feb: 1,
    mar: 2,
  ...
  function parseChapterNumber2(value) {
    const match = value.match(/(\d+(?:\.\d+)?)\s*$/);
    return match === null ? void 0 : Number.parseFloat(match[1] ?? "");
  }
  function parseListingChapter(item) {
    ...
    return { chapterId: ref.chapterId, label, chapNum: parseChapterNumber2(label) };
  }
  function parseMangaList($) {
    ...
    $("li").each((_, element) => {
      ...
      seen.add(mangaId);
      const genres = item.find("p.keyWord a").toArray().map((genre) => cleanText($(genre).text())).filter((genre) => genre.length > 0);
      let author;
      let status;
      let views;
      let rank;
      ...
      items.push({
        ...
        author,
        status,
        views: views !== void 0 && Number.isFinite(views) ? views : void 0,
        rank: rank !== void 0 && Number.isFinite(rank) ? rank : void 0,
  ...
  function parseHasNextPage($) {
    return $("a.next").toArray().some((element) => !($(element).attr("href") ?? "").startsWith("javascript"));
  }
  function labelledListText($, info, label) {
    let result = "";
  ...
  function labelledLinkText($, info, label) {
    let result = "";
  ...
  function parseMangaDetails($, mangaId) {
    ...
    const secondaryTitles = labelledListText($, info, "alternative").replace(/^alternative\s*name\s*:?/i, "").split(/\s*;\s*/).map(cleanText).filter((value) => value.length > 0 && value.toLowerCase() !== title.toLowerCase());
    const tagGroups = [];
    if (genres.length > 0) {
      tagGroups.push({
        id: "genres",
    ...
    return {
      ...
      mangaInfo: {
        ...
        author: author.length > 0 ? author : void 0,
        artist: artist.length > 0 ? artist : void 0,
        ...
        rating: Number.isFinite(rating) ? Math.min(1, Math.max(0, rating / 5)) : void 0,
        tagGroups
      }
    };
  }
  function parseChapters($, sourceManga) {
    const chapters = [];
    const seen = /* @__PURE__ */ new Set();
    ...
    entries.forEach((element, index) => {
      ...
      const ref = parseChapterRef(link.attr("href"));
      if (ref === void 0 || seen.has(ref.chapterId)) {
        return;
      }
      seen.add(ref.chapterId);
      const linkText = cleanText(link.text());
      const extra = item.find("span").toArray().map((span) => $(span)).filter((span) => {
        const className = span.attr("class") ?? "";
        return !className.includes("time") && !className.includes("new");
      }).map((span) => cleanText(span.text())).filter((text) => text.length > 0).join(" ");
      const chapNum = parseChapterNumber2(linkText);
      ...
      chapters.push({
        ...
        chapNum: chapNum ?? 0,
        title: extra.length > 0 ? extra : chapNum === void 0 ? cleanText(linkText.replace(mangaTitle, "")) : void 0,
  ...
  var PAGE_OPTION_SELECTOR = "select#top_chapter_list ~ div.page_select option";
  var FALLBACK_PAGE_OPTION_SELECTOR = "div.manga_read_footer div.page_select option";
  function parseChapterPageUrls($) {
  ...
  var SEQUENTIAL_IMAGE = /^(.*\/)([^/\d]*)(\d+)(\.[A-Za-z0-9]+)$/;
  function buildSequentialImageUrls(imageUrl, imagePage, totalPages) {
    ...
    return Array.from(
      { length: totalPages },
      (_, index) => `${directory}${prefix}${String(first + index).padStart((digits ?? "").length, "0")}${extension}`
    );
  }
  function parseViewerImage($) {
    return imageUrlFrom($("div#viewer img, img#image, source#image").first());
  }
  function parseViewerImages($) {
    return $("div#viewer img").toArray().map((element) => imageUrlFrom($(element))).filter((url) => url.length > 0);
  }
  ...
  function chapterLabel(chapter) {
    ...
    return chapter.chapNum !== void 0 ? `Ch. ${chapter.chapNum}` : chapter.label || void 0;
  }
  function joinDetails(parts) {
    const joined = parts.filter((part) => part !== void 0 && part.length > 0).join(" \u2022 ");
    return joined.length > 0 ? joined : void 0;
  }
  function toDiscoverItem(item) {
    return {
      mangaId: item.mangaId,
      title: item.title,
      imageUrl: item.imageUrl,
      subtitle: joinDetails([
        item.rating === void 0 ? void 0 : `\u2605 ${item.rating.toFixed(2)}`,
  ...
  function toTopItem(item) {
    return {
      ...
      imageUrl: item.imageUrl,
      subtitle: joinDetails([
        item.rank === void 0 ? void 0 : `Rank ${item.rank}`,
        item.views === void 0 ? void 0 : `${formatCount(item.views)} views`,
        item.status
      ])
    };
  }
  function toChapterUpdateItem(item) {
    if (item.chapter === void 0) {
      return void 0;
    }
    return {
      mangaId: item.mangaId,
  ...
  var MangaTown = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
      ...
      this.readerCache = /* @__PURE__ */ new Map();
    }
    getMangaShareUrl(mangaId) {
    ...
    async getDiscoverSections() {
      ...
      const wanted = chosen.length > 0 ? new Set(chosen) : void 0;
      return SECTION_ORDER.filter((id) => wanted === void 0 || wanted.has(id)).map(
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const request = {
        name: title.length > 0 ? title : void 0,
        ...
        demographic: demographic === void 0 ? void 0 : GENRE_TITLES.get(demographic),
        isCompleted: status === "completed" ? "1" : status === "ongoing" ? "0" : void 0
      };
    ...
    async getPages(chapter) {
      ...
      const cacheKey2 = `${mangaId}/${chapter.chapterId}`;
      const cached = this.readerCache.get(cacheKey2);
      if (cached !== void 0) {
      ...
      const pages = pageUrls.length > 0 ? await this.readPagedChapter(document, pageUrls) : parseViewerImages(document);
      ...
      if (valid.length === pages.length) {
        this.readerCache.set(cacheKey2, valid);
      }
    ...
    async listing(url, page, map) {
      ...
      return {
        ...
        metadata: parseHasNextPage(document) ? { page: page + 1 } : void 0
      };
    }
    /**
     ...
     */
    async readPagedChapter(firstPage, pageUrls) {
      const firstImage = parseViewerImage(firstPage);
      if (pageUrls.length === 1) {
        return [firstImage];
      }
      const derived = firstImage.length > 0 ? buildSequentialImageUrls(firstImage, 1, pageUrls.length) : void 0;
      if (derived !== void 0) {
        ...
        const imagePath = (url) => url.replace(/^https?:\/\/[^/]+/i, "");
     ...
     * connections at once.
     */
    async fetchPageImages(firstPage, pageUrls) {
      const images = [parseViewerImage(firstPage)];
  ...
  return __toCommonJS(MangaTown_exports);
})();
