...
var _Sources = (() => {
  ...
  // src/MyReadingManga/MyReadingManga.ts
  var MyReadingManga_exports = {};
  __export(MyReadingManga_exports, {
    MyReadingManga: () => MyReadingManga,
    MyReadingMangaInfo: () => MyReadingMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/MyReadingManga/models.ts
  var DOMAIN = "https://myreadingmanga.info";
  var SECTIONS = {
    POPULAR: "popular",
    LATEST: "latest",
    MANGA: "manga",
  ...
  var LANGUAGES_KEY = "myreadingmanga.languages";
  var HIDDEN_GENRES_KEY = "myreadingmanga.hiddenGenres";
  var HIDDEN_TAGS_KEY = "myreadingmanga.hiddenTags";
  var VISIBLE_SECTIONS_KEY = "myreadingmanga.sections";
  var SETTINGS_KEYS = [
    LANGUAGES_KEY,
    HIDDEN_GENRES_KEY,
    HIDDEN_TAGS_KEY,
  ...
  var DISCOVER_SECTIONS = [
    ...
    { id: SECTIONS.LATEST, title: "Latest", type: "simpleCarousel" /* simpleCarousel */ },
    { id: SECTIONS.MANGA, title: "Manga", type: "simpleCarousel" /* simpleCarousel */ },
    { id: SECTIONS.BARA, title: "Bara", type: "simpleCarousel" /* simpleCarousel */ },
    { id: SECTIONS.RANDOM, title: "Random", type: "simpleCarousel" /* simpleCarousel */ }
  ];
  var SECTION_IDS = DISCOVER_SECTIONS.map((section) => section.id);
  var SECTION_OPTIONS = DISCOVER_SECTIONS.map((section) => ({
    id: section.id,
    title: section.title
  }));

  // src/MyReadingManga/network.ts
  var CHALLENGE_PROMPT_INTERVAL = 6e4;
  var MyReadingMangaInterceptor = class extends PaperbackInterceptor {
    constructor() {
      super(...arguments);
    ...
    async interceptResponse(request, response, body) {
      ...
      const text = request.url.startsWith(DOMAIN) && contentType.includes("text/html") ? body.text : "";
  ...
  function pagedUrl(base, page) {
    ...
    return query !== void 0 ? `${pagedPath}?${query}` : pagedPath;
  }
  function fetchListingPage(path, page) {
  ...
  function fetchMangaPage(mangaId, part) {
    return Application.fetchDocument({
      url: `${DOMAIN}/${mangaId}/${part !== void 0 && part > 1 ? `${part}/` : ""}`
    });
  }
  ...
  // src/MyReadingManga/parsers.ts
  var IMAGE_EXTENSION = /\.(jpg|jpeg|png|webp|gif|avif)/i;
  ...
  function bracketAuthor(raw) {
    return /\[([^\]]+)\]/.exec(raw)?.[1]?.trim();
  }
  function imageFrom(img) {
    for (const attribute2 of ["data-src", "data-cfsrc", "src", "data-lazy-src"]) {
      const value = (img.attr(attribute2) ?? "").trim();
  ...
  function stripThumbnailSize(src) {
    return src.replace(/-\d+x\d+(\.\w+)$/, "$1");
  }
  function toMangaId(href) {
    return href.replace(/^https?:\/\/[^/]+\//, "").replace(/[?#].*$/, "").replace(/\/+$/, "");
  }
  function parseListing($, filter = {}) {
    const cards = [];
    ...
    for (const element of $("article, div.post, div.item, ul.wpp-list li").toArray()) {
      ...
      if (languages.length > 0 && classes.includes("lang-") && !languages.some((language) => classList.includes(`lang-${language}`))) {
        continue;
      }
      const link = article.find(".entry-title a, h1 a, h2 a, h3 a, a.wpp-post-title, a[rel=bookmark]").first();
  ...
  function parseMangaDetails($, mangaId) {
    ...
    const thumbnailUrl = schemaThumbnail($) || imageFrom($("img.img-myreadingmanga").first()) || imageFrom($("div.entry-content img").first());
    ...
    const scanGroups = $(".entry-terms a[href*='/group/']").toArray().map((element) => $(element).text().trim()).filter((name) => name.length > 0);
    const paragraphs = $("div.entry-content p").toArray().map((element) => Application.decodeHTMLEntities($(element).text().trim())).filter((text) => text.length > 0 && !text.includes("|"));
    const synopsis = [scanGroups.length > 0 ? `Scanlated by: ${scanGroups.join(", ")}` : "", ...paragraphs].filter((part) => part.length > 0).join("\n");
  ...
  function parseChapters($, sourceManga) {
    ...
    const publishDate = publishDateText.length > 0 ? new Date(publishDateText) : void 0;
    const partNumbers = $("a.page-numbers:not(.next):not(.prev)").toArray().map((element) => parseInt($(element).text().trim(), 10)).filter((value) => !isNaN(value));
    ...
    for (let part = lastPart; part >= 1; part -= 1) {
      chapters.push({
        ...
        title: `Part ${part}`,
        volume: 0,
        publishDate: publishDate !== void 0 && !isNaN(publishDate.getTime()) ? publishDate : void 0
      });
    }
    return chapters;
  }
  function parsePages($, chapter) {
    const pages = [];
    const seen = /* @__PURE__ */ new Set();
  ...
  function parseFilterTaxonomies($) {
    const widgetIds = {
      genre: "genre",
      category: "category",
      tag: "tag",
    ...
    for (const element of $("aside.ep-search-sidebar div.ep-filter-widget").toArray()) {
      ...
      const options = widget.find("div.term").toArray().flatMap((term) => {
  ...
  // src/MyReadingManga/MyReadingManga.ts
  var MyReadingMangaInfo = sourceInfo({
    name: "MyReadingManga",
  ...
  var MyReadingManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getSettingsSections() {
      const taxonomies = await this.getTaxonomies().catch((error) => {
        if (error instanceof CloudflareError) {
          throw error;
        }
        return {};
      ...
      const sections = [
        {
          id: "languages",
          header: "Languages",
          footer: "Only entries in these languages appear on the home page. Leave everything unticked to show them all.",
          rows: [
            selectRow("languages", {
    ...
    async getDiscoverSections() {
      ...
      return DISCOVER_SECTIONS.filter((section) => wanted === void 0 || wanted.has(section.id));
    }
     ...
     */
    async getFilterSections() {
      const taxonomies = await this.getTaxonomies().catch(() => ({}));
      const sections = [
        {
          id: FILTERS.SORT,
          title: "Sort by",
    ...
    getTaxonomies() {
      this.taxonomies ??= fetchSearchPage({ page: 1, query: "", sort: "rand", facets: /* @__PURE__ */ new Map() }).then(parseFilterTaxonomies).catch((error) => {
        this.taxonomies = void 0;
        throw error;
      });
  ...
  return __toCommonJS(MyReadingManga_exports);
})();
