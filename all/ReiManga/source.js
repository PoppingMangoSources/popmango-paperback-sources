...
var _Sources = (() => {
  ...
  // src/ReiManga/ReiManga.ts
  var ReiManga_exports = {};
  __export(ReiManga_exports, {
    ReiManga: () => ReiManga,
    ReiMangaInfo: () => ReiMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
    ...
    return App.createSourceManga({
      ...
      mangaInfo: App.createMangaInfo({
        ...
        titles: [info.primaryTitle, ...info.secondaryTitles].filter(
          (title2, index, all) => title2.length > 0 && all.indexOf(title2) === index
        ),
        desc: info.synopsis,
        status: info.status,
  ...
  function settingsMenu(title2, sections) {
    ...
    return App.createDUISection({
      id: "settings",
      isHidden: false,
      rows: async () => [App.createDUINavigationButton({ id: "settings_button", label: title2, form })]
    });
  }
  ...
  // src/ReiManga/models.ts
  var DOMAIN = "https://reimanga.net";
  var PAGE_SIZE = 24;
  var BASE_URL_KEY = "reimanga.baseUrl";
  ...
  function filterTag(section, id, title2) {
    return { id: `${section}${FILTER_SEPARATOR}${id}`, title: title2 };
  }
  ...
  var ADULT_GENRES = ["ecchi", "smut", "adult", "mature", "yaoi", "yuri", "hentai"];

  // src/ReiManga/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif|bmp)([?#]|$)/i;
  var userAgent;
  function getUserAgent() {
    return userAgent ??= Application.getDefaultUserAgent();
  }
  var ReiMangaInterceptor = class extends PaperbackInterceptor {
    ...
    async interceptRequest(request) {
      ...
      return {
        ...
        headers: {
          ...
          accept: isImage ? "image/avif,image/webp,image/apng,image/png,image/svg+xml,*/*;q=0.8" : request.headers.accept ?? "*/*"
        },
        cookies: {
          ...request.cookies,
  ...
  function fetchJson(url) {
    return Application.fetchJSON({ url, headers: { accept: "application/json" } });
  }
  function fetchFlight(url) {
    return Application.fetchText({ url, headers: { rsc: "1", accept: "text/x-component,*/*" } });
  }
  function buildSearchUrl(baseUrl, request) {
    const params = [`page=${request.page}`, `limit=${PAGE_SIZE}`];
    if (request.term !== void 0 && request.term.length > 0) {
  ...
  function mangaIdFor(manga) {
    return manga.name_url !== void 0 ? `${manga.name_url}-${manga.id}` : String(manga.id);
  }
  function numericIdFrom(mangaId) {
    return /(\d+)$/.exec(mangaId)?.[1];
  }
  function tagNames(values) {
    return (values ?? []).map((value) => typeof value === "string" ? value : value.name ?? "").map(cleanText).filter((name) => name.length > 0);
  }
  function genreNames(manga) {
    const named = tagNames(manga.genres);
    if (named.length > 0) {
      return named;
    }
    return (manga.genre_slugs ?? "").split(",").map((slug) => slug.trim()).filter((slug) => slug.length > 0).map((slug) => slug.replace(/-/g, " ").replace(/\b\w/g, (character) => character.toUpperCase()));
  }
  function contentRatingFor(manga) {
    if (manga.is_adult) {
      return "ADULT" /* ADULT */;
    }
    const names = [...genreNames(manga), ...tagNames(manga.tags)].map((name) => name.toLowerCase());
    return names.some((name) => ADULT_GENRES.includes(name)) ? "ADULT" /* ADULT */ : "MATURE" /* MATURE */;
  }
  function coverUrlFor(baseUrl, manga) {
  ...
  function toDiscoverItem(baseUrl, manga, detail, rank) {
    const rating = ratingFor(manga);
    const lead = detail === "reads" ? joinDetails([chapterLabel(manga), compactCount(manga.recent_reads ?? manga.view_count)]) : detail === "rating" ? joinDetails([rating === void 0 ? void 0 : `${rating.toFixed(1)} \u2605`, chapterLabel(manga)]) : detail === "updated" ? joinDetails([
      chapterLabel(manga),
    ...
    return {
      ...
      subtitle: joinDetails([rank === void 0 ? void 0 : `#${rank}`, lead])
    };
  }
  function toSearchResultItem(baseUrl, manga) {
    const rating = ratingFor(manga);
    return {
      mangaId: mangaIdFor(manga),
      title: title(manga),
      imageUrl: coverUrlFor(baseUrl, manga),
      subtitle: joinDetails([rating === void 0 ? void 0 : `${rating.toFixed(1)} \u2605`, chapterLabel(manga)]),
      contentRating: contentRatingFor(manga)
    };
  }
  function toSourceManga2(baseUrl, manga, mangaId) {
    const seen = /* @__PURE__ */ new Set();
    const group = (names) => names.flatMap((name) => {
    ...
    const rating = ratingFor(manga);
    const alternate = manga.alt_title === null || manga.alt_title === void 0 ? "" : cleanText(manga.alt_title);
    const author = tagNames(manga.authors).join(", ");
    return {
      mangaId,
      mangaInfo: {
        primaryTitle: title(manga),
        secondaryTitles: alternate.length > 0 ? alternate.split(/[,;|]/).map((value) => value.trim()).filter((value) => value.length > 0) : [],
        thumbnailUrl: coverUrlFor(baseUrl, manga),
        synopsis: manga.description === null || manga.description === void 0 ? "" : cleanText(manga.description),
        ...
        rating: rating === void 0 ? void 0 : Math.min(1, Math.max(0, rating / 10)),
        contentRating: contentRatingFor(manga),
        tagGroups: [
  ...
  function extractFlight(body, key) {
    ...
    while (from >= 0) {
      for (let start = from; start >= 0; start -= 1) {
        ...
        if (slice === void 0 || slice.length < marker.length || !slice.includes(marker)) {
          continue;
        }
        try {
  ...
  function parseChapters(body, sourceManga) {
    const entries = extractFlight(body, "chapters")?.chapters ?? [];
      ...
      return [
        {
          ...
          title: name.length > 0 && !/^(?:ch\.?|chapter)\s*[\d.]+$/i.test(name) ? name : void 0,
          volume: 0,
  ...
  function parseChapterPages(body, chapter) {
    const pages = (extractFlight(body, "images")?.images ?? []).slice().sort((left, right) => (left.page_number ?? 0) - (right.page_number ?? 0)).map((image) => (image.image_url ?? image.url ?? "").trim()).filter((url) => url.length > 0);
    if (pages.length === 0) {
  ...
  var ReiManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: SECTIONS.MOST_READ, title: "Most Read This Week", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.NEW, title: "New Manga", type: "simpleCarousel" /* simpleCarousel */ },
        { id: SECTIONS.LATEST, title: "Latest Updates", type: "simpleCarousel" /* simpleCarousel */ },
    ...
    async getDiscoverSectionItems(section) {
      ...
      switch (section.id) {
        case SECTIONS.FEATURED: {
          const list = await fetchJson(`${base}/api/manga/trending?limit=10&full=1`);
        ...
        case SECTIONS.NEW: {
          const list = await fetchJson(`${base}/api/manga/new?limit=12`);
          return { items: (list.data ?? []).map((manga) => toDiscoverItem(base, manga, "chapter")) };
        }
        case SECTIONS.LATEST: {
          const list = await fetchJson(`${base}/api/manga/latest-updates?limit=18`);
    ...
    async getSearchResultItems(query, metadata) {
      ...
      if (period !== void 0) {
        const list2 = await fetchJson(
          `${base}/api/manga/most-read?limit=30&period=${encodeURIComponent(period)}`
        );
        return { items: list2.map((manga) => toSearchResultItem(base, manga)) };
      }
      ...
      return {
        ...
        metadata: current < total ? { page: page + 1 } : void 0
      };
    }
    async getMangaInfo(mangaId) {
      ...
      const details = await fetchJson(`${this.baseUrl}/api/manga/${numericId}`);
      if (details.manga === void 0) {
        throw new Error(`Unable to read the details for ${mangaId}.`);
      }
      return toSourceManga2(this.baseUrl, details.manga, mangaId);
    }
  ...
  return __toCommonJS(ReiManga_exports);
})();
