...
var _Sources = (() => {
  ...
  // src/RinkoComics/RinkoComics.ts
  var RinkoComics_exports = {};
  __export(RinkoComics_exports, {
    RinkoComics: () => RinkoComics,
    RinkoComicsInfo: () => RinkoComicsInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  var ApplicationRuntime = class {
    ...
    async fetchJSON(init, retries = DEFAULT_RETRIES) {
      const text2 = await this.fetchText(init, retries);
      try {
  ...
  // src/RinkoComics/models.ts
  var DOMAIN = "https://rinkocomics.com";
  var AJAX_ENDPOINT = `${DOMAIN}/wp-admin/admin-ajax.php`;
  var LOCK_SUFFIX = "#lock";
  var CHAPTER_SELECTOR = "li.chapter, div.chapter, a.chapter-item";
  var HIDE_LOCKED_KEY = "rinkocomics.hideLocked";
  var SETTINGS_KEYS = [HIDE_LOCKED_KEY];
  var SECTIONS = {
    HOT: "hot",
    PINNED: "pinned",
    LATEST: "latest"
  ...
  // src/RinkoComics/network.ts
  var IMAGE_URL2 = /\.(jpe?g|png|webp|gif|avif|bmp|svg)([?#]|$)/i;
  var RinkoComicsInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
      const accept = IMAGE_URL2.test(request.url) ? "image/avif,image/webp,image/apng,image/png,image/svg+xml,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8";
      return {
        ...request,
        headers: {
          ...request.headers,
  ...
  async function fetchMoreChaptersHtml(comicId, offset, nonce) {
    const body = [
    ...
    const parsed = await Application.fetchJSON({
      url: AJAX_ENDPOINT,
      method: "POST",
      headers: {
  ...
  function toSafeId(slug) {
    return slug.replace(/[^A-Za-z0-9._\-@()[\]%?#+=/&:]/g, (character) => {
      ...
      return encoded !== character ? encoded : `%${character.charCodeAt(0).toString(16).toUpperCase().padStart(2, "0")}`;
    });
  }
  ...
  function parsePath(href) {
    ...
    const slug = cleaned.startsWith("http") ? cleaned.replace(/^https?:\/\/[^/]+\//, "") : cleaned.replace(/^\/+/, "");
  ...
  function parseRelativeDate(value) {
    ...
    return factor === void 0 ? void 0 : new Date(Date.now() - amount * factor);
  }
  ...
  function toHotItems($) {
    ...
    for (const element of $(".popular-comics .comic-card-popular").toArray()) {
      ...
      const [views, chapters] = card.find(".comic-stats .stat").toArray().map((stat) => $(stat).text().trim());
      items.push({
        mangaId,
        title,
        imageUrl: imageUrlFrom(card.find(".comic-cover img").first()),
        subtitle: joinDetails([
          rank.length > 0 ? `#${rank}` : void 0,
          views,
          chapters === void 0 || chapters.length === 0 ? void 0 : `${chapters} ch`
        ])
      });
    }
    return items;
  }
  function toPinnedItems($) {
    const items = [];
    ...
    for (const element of $("a.pinned-comic-card").toArray()) {
      ...
      items.push({
        ...
        subtitle: badge.length > 0 ? badge : void 0
      });
    }
    return items;
  }
  function toLatestItems($) {
    const items = [];
    for (const element of $(".latest-releases .comic-card").toArray()) {
      ...
      const chapter = card.find("a.chapter-item").toArray().map((entry) => $(entry)).find((entry) => !isLocked(entry) && (entry.attr("href") ?? "").includes("/chapter/"));
      if (chapter === void 0) {
        continue;
      }
      ...
      items.push({
        ...
        subtitle: label.length > 0 ? label : void 0,
  ...
  function parseMangaDetails($, mangaId) {
    ...
    return {
      ...
      mangaInfo: {
        ...
        synopsis: text(
          $(".comic-synopsis").first().text() || $("meta[property=og:description]").first().attr("content") || ""
        ),
        author: authors[0],
        artist: authors[1],
  ...
  function parseChapterElements($, elements, sourceManga) {
    const chapters = [];
    elements.each((_, element) => {
      ...
      const name = text(
        row.find(".chapter-number").first().text() || row.find(".ch-name").first().text() || row.find(".chapter-side-title").first().text() || (row.attr("data-title") ?? "").trim() || row.find("label").first().text()
      );
      let title = name.replace(/^chapter\s+\d+(?:\.\d+)?(?:\s*[-:]\s*)?/i, "").trim();
      let chapterId = rawUrl.length > 0 ? parsePath(rawUrl) : `locked-${postId}`;
      if (locked) {
        title = title.length > 0 ? `${title} \u{1F512}` : "\u{1F512}";
      ...
      chapters.push({
        ...
        title,
        volume: 0,
        chapNum: parseChapterNumber2(name),
        publishDate: parseDate2(row.find(".chapter-date").first().text()),
        langCode: "\u{1F1EC}\u{1F1E7}"
      });
    });
    return chapters;
  }
  function finaliseChapters(chapters) {
  ...
  // src/RinkoComics/RinkoComics.ts
  var RinkoComicsInfo = sourceInfo({
    name: "RinkoComics",
  ...
  var RinkoComics = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getDiscoverSections() {
      return [
        { id: SECTIONS.HOT, title: "Hot This Week", type: "featured" /* featured */ },
        { id: SECTIONS.PINNED, title: "Editor's Choice", type: "prominentCarousel" /* prominentCarousel */ },
    ...
    async getSearchResultItems(query, metadata) {
      ...
      if (genres !== void 0 && genres.length > 0) {
  ...
  return __toCommonJS(RinkoComics_exports);
})();
