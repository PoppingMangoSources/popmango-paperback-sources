...
var _Sources = (() => {
  ...
  // src/RokariComics/RokariComics.ts
  var RokariComics_exports = {};
  __export(RokariComics_exports, {
    RokariComics: () => RokariComics,
    RokariComicsInfo: () => RokariComicsInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/RokariComics/models.ts
  var DOMAIN = "https://rokaricomics.com";
  var MANGA_DIR = "manga";
  var NEXT_PAGE_SELECTOR = "div.hpage .r, div.pagination .next, a.next.page-numbers";
  var BASE_URL_KEY = "rokaricomics.baseUrlOverride";
  var USE_POST_IDS_KEY = "rokaricomics.usePostIds";
  var SETTINGS_KEYS = [BASE_URL_KEY, USE_POST_IDS_KEY];
  var SECTIONS = {
    FEATURED: "featured",
    POPULAR: "popular",
  ...
  // src/RokariComics/network.ts
  var RokariComicsInterceptor = class extends PaperbackInterceptor {
    constructor(getDomain) {
      super();
  ...
  // src/RokariComics/parsers.ts
  function cleanText(value) {
    return Application.decodeHTMLEntities(value).trim();
  }
  function getImageSrc(image) {
    let src = image?.attr("data-src") ?? image?.attr("data-lazy-src") ?? image?.attr("srcset")?.split(" ")[0] ?? image?.attr("src") ?? image?.attr("data-cfsrc") ?? "";
    src = src.split("?resize")[0] ?? "";
    src = src.replace(/^\/\//, "https://").replace(/^\//, "https:/");
    return encodeURI(decodeURI(src.trim()));
  }
  function idCleaner(value) {
  ...
  function parseChapterDate(text) {
    ...
    return isNaN(direct.getTime()) ? void 0 : direct;
  }
  ...
  function labelledValue($, label) {
    return cleanText(
      $(
        `span:contains(${label}), .fmed b:contains(${label})+span, .imptdt:contains(${label}) i, tr td:contains(${label}) + td`
      ).contents().remove().last().text()
    );
  }
  function parseMangaDetails($, mangaId) {
    const titles = [cleanText($("h1.entry-title").text())];
    const altTitles = $(
      "span:contains(Alternative Titles), b:contains(Alternative Titles)+span, .imptdt:contains(Alternative Titles) i, h1.entry-title+span"
    ...
    return {
      ...
      mangaInfo: {
        ...
        author: author.length > 0 && author !== "Unknown" ? author : void 0,
        artist: artist.length > 0 && artist !== "Unknown" ? artist : void 0,
  ...
  function parseChapterList($, sourceManga) {
    const chapters = [];
  ...
  function parseChapterPages($, chapter) {
    const script = $("script").toArray().map((element) => $(element).html() ?? "").find((html) => html.includes("ts_reader.run"));
    if (script === void 0) {
  ...
  function parseTags($) {
    ...
    for (let index = 0; index < sections.length; index += 1) {
      ...
      if (dropdown === void 0 || section === void 0) {
        continue;
      }
  ...
  function parseSearchResults($) {
    ...
    for (const element of $("div.bs", "div.listupd").toArray()) {
      ...
      results.push({
        ...
        subtitle: cleanText($("div.epxs", element).text()) || void 0
      });
    }
    return results;
  }
  function parseRankingList($, range) {
    const results = [];
    for (const element of $(`div.serieslist.pop.wpop-${range} li`).toArray()) {
      ...
      results.push({
        ...
        title,
        imageUrl: getImageSrc($("img", element)),
        subtitle: $("div.leftseries span", element).first().text().replace(/^\s*Genres:\s*/i, "").trim() || void 0
      });
    }
    return results;
  }
  ...
  // src/RokariComics/RokariComics.ts
  var RokariComicsInfo = sourceInfo({
    name: "Rokari Comics",
  ...
  var RokariComics = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    getSettingsSections() {
      return [
        ...
        },
        {
          id: "ids",
          header: "Series ids",
          footer: "The site renames series from time to time, which loses them from a saved library. Post ids survive a rename, at the cost of one extra lookup per series.",
          rows: [
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: SECTIONS.POPULAR, title: "Popular Today", type: "prominentCarousel" /* prominentCarousel */ },
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const hasFilters = genres.length > 0 || status !== void 0 || type !== void 0 || sort !== void 0;
    ...
    async getPages(chapter) {
      ...
      const row = $("div#chapterlist").find("li").toArray().find((element) => $(element).attr("data-num") === chapter.chapterId);
      if (row === void 0) {
    ...
    // -----------------------------------------------------------------
    mangaUrl(mangaId) {
      return this.usePostIds ? `${this.baseUrl}/?p=${mangaId}/` : `${this.baseUrl}/${MANGA_DIR}/${mangaId}/`;
    }
    /** The id to store for a card, which depends on the ids setting. */
    async mangaIdFor(card) {
      if (!this.usePostIds) {
        return card.slug;
      }
      if (card.postId !== void 0 && !isNaN(Number(card.postId))) {
        return card.postId;
      }
    ...
    async parseFeatured($) {
      ...
      for (const slide of $("div.slider-wrapper div.swiper-slide").toArray()) {
        ...
        const chapterLabel = ($("span.chapter, div.chapter, span.fivchap, span.epxs, div.epxs", slide).first().text() || ($(slide).text().match(/(?:Chapter|Ch\.?)\s*[\d.]+/i)?.[0] ?? "")).replace(/\s+/g, " ").trim();
        items.push({
          ...
          imageUrl,
          title,
          subtitle: chapterLabel.length > 0 ? chapterLabel : void 0
        });
      }
      return items;
    }
    async parseLatest($) {
      ...
      for (const element of $(".bixbox:has(h2:contains(Latest)) .bs .bsx").toArray()) {
        ...
        const chapterId = /([\d.]+)\s*$/.exec(chapterLabel)?.[1] ?? /chapter-(\d+(?:-\d+)?)\/?$/.exec(chapterAnchor.attr("href") ?? "")?.[1]?.replace(/-/g, ".") ?? "";
        if (chapterId.length === 0) {
          continue;
        }
        ...
        items.push({
          ...
          imageUrl: getImageSrc($("img", element)),
          title,
          subtitle: ageText.length > 0 ? `${chapterLabel} \xB7 ${ageText}` : chapterLabel || void 0,
          publishDate: isNew ? /* @__PURE__ */ new Date() : parseRelativeDate(rawTime)
        });
      }
      return items;
    }
    /** The plainer home page strips, which all share one shape. */
    async parseStrip($, selector, type) {
      const items = [];
      for (const element of $(selector).toArray()) {
        ...
        items.push({
          ...
          imageUrl: getImageSrc($("img", element)),
          title,
          subtitle: cleanText($("div.epxs", element).first().text()).replace(/\s+/g, " ") || void 0
        });
      }
      return items;
    }
    ...
    getTags() {
      ...
      if (this.tags === void 0) {
        const entry = {
          base,
          promise: fetchPage(`${base}/${MANGA_DIR}/`).then(
            ($) => parseTags($).filter((section) => section.tags.length > 0).map((section) => ({
              id: section.id,
  ...
  return __toCommonJS(RokariComics_exports);
})();
