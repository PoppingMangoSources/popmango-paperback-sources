...
var _Sources = (() => {
    ...
    "node_modules/@paperback/types/lib/base/Source.js"(exports) {
      ...
      function convertTime(timeAgo) {
        ...
        if (timeAgo.includes("minutes")) {
          time2 = new Date(Date.now() - trimmed * 6e4);
        } else if (timeAgo.includes("hours")) {
          time2 = new Date(Date.now() - trimmed * 36e5);
        } else if (timeAgo.includes("days")) {
          time2 = new Date(Date.now() - trimmed * 864e5);
        } else if (timeAgo.includes("year") || timeAgo.includes("years")) {
          time2 = new Date(Date.now() - trimmed * 31556952e3);
        } else {
  ...
  // src/TempleScan/TempleScan.ts
  var TempleScan_exports = {};
  __export(TempleScan_exports, {
    TempleScan: () => TempleScan,
    TempleScanInfo: () => TempleScanInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/TempleScan/models.ts
  var DOMAIN = "https://templetoons.com";
  var API_URL = "https://api.templetoons.com/api";
  var PAID_CHAPTER_SUFFIX = "#paid";
  var PAGE_SIZE = 20;
  var SHOW_PAID_CHAPTERS_KEY = "templescan.showPaidChapters";
  var SETTINGS_KEYS = [SHOW_PAID_CHAPTERS_KEY];
  var FILTERS = {
    SORT: "sort",
    STATUS: "status",
    TRENDING: "trending"
  ...
  // src/TempleScan/network.ts
  var FIRST_PARTY = /^https:\/\/(?:[a-z0-9-]+\.)*templetoons\.com\//i;
  var TempleScanInterceptor = class extends PaperbackInterceptor {
    async interceptRequest(request) {
  ...
  function fetchSeriesPage(mangaId) {
  ...
  // src/TempleScan/parsers.ts
  function sliceJson(payload, start) {
    let depth = 0;
    let inString = false;
    let escaped = false;
  ...
  function decodeEscaped(payload) {
    return payload.replace(/\\"/g, '"').replace(/\\\\/g, "\\");
  }
  function scanByKey(payload, key, matches) {
    const marker = `"${key}":`;
  ...
  function extractByKey(payload, key, matches) {
  ...
  function parseDirectory(payload) {
    ...
    for (const text of [payload, decodeEscaped(payload)]) {
      ...
      while (index !== -1) {
        ...
        if (raw !== void 0 && raw.length > 40) {
          try {
            const value = JSON.parse(raw);
            if (Array.isArray(value) && value.length > directory.length && value.every((entry) => entry !== null && typeof entry === "object") && value[0]?.series_slug !== void 0 && value[0]?.title !== void 0) {
              directory = value;
            }
          } catch {
          }
  ...
  function resolveFlightRef(payload, ref) {
    for (const text of [payload, decodeEscaped(payload)]) {
  ...
  function parseSeriesData(payload, mangaId) {
    const data = extractByKey(
      payload,
      "seriesData",
  ...
  function cleanDescription(raw) {
    const text = raw.includes("#") ? raw.slice(0, raw.indexOf("#")).replace(/[\w\s]+:?\s*$/, "").trim() : raw;
  ...
  function toSourceManga2(data, mangaId) {
    const description = data.description ?? "";
    const tagTitles = [
      data.badge ?? "",
      data.release_year !== null && data.release_year !== void 0 ? String(data.release_year) : "",
      ...(data.tag_series ?? []).map((wrapper) => wrapper.tag?.name ?? ""),
      ...[...description.matchAll(/#(\w+)/g)].map((match) => match[1] ?? "")
    ].filter((title) => title.length > 0);
    const seen = /* @__PURE__ */ new Set();
    const tags = tagTitles.flatMap((title) => {
  ...
  function chapterTitle(chapter) {
    const title = ((chapter.chapter_title ?? "").trim() || (chapter.chapter_name ?? "").trim()).replace(/^chapter\s+\d+(?:\.\d+)?(?:\s*[-:]\s*)?/i, "").trim();
    if (!chapterIsPaid(chapter)) {
      return title;
    }
    return title.length > 0 ? `${title} \u{1F512}` : "\u{1F512}";
  }
  function updateCardSubtitle(chapter) {
    const number = chapterNumber(chapter);
    return [number > 0 ? `Chapter ${number}` : "", chapterTitle(chapter)].filter((part) => part.length > 0).join(" \u2022 ");
  }
  function parseChapters(data, sourceManga, showPaidChapters) {
    return (data.Season ?? []).flatMap((season) => season.Chapter ?? []).filter((chapter) => chapter.chapter_slug && (showPaidChapters || !chapterIsPaid(chapter))).map((chapter) => ({
  ...
  function parseChapterPages(payload, chapter) {
    const pages = extractByKey(
      payload,
      "pages",
      (value) => Array.isArray(value) && (value.length === 0 || typeof value[0] === "string")
    );
    if (pages === void 0 || pages.length === 0) {
      throw new Error(`No pages were found for chapter ${chapter.chapterId}.`);
    }
    return { id: chapter.chapterId, mangaId: chapter.sourceManga.mangaId, pages };
  }
  function parseHomeSections(payload) {
    return {
      newSeries: extractByKey(
        payload,
        "data",
        (value) => Array.isArray(value) && value.length > 0 && Boolean(value[0]?.series_slug) && value[0]?.Chapter === void 0
      ) ?? [],
      updates: extractByKey(
        payload,
        "series",
  ...
  function toNewSeriesItems(series) {
    return series.map((entry) => ({
      type: "simpleCarouselItem",
      mangaId: entry.series_slug,
      title: entry.title,
      imageUrl: entry.thumbnail ?? "",
      subtitle: entry.badge ?? void 0
    }));
  }
  function toUpdateItems(updates) {
  ...
  function parseFeatured(response) {
    let parsed;
    try {
  ...
  function withFeaturedCovers(entries, directory) {
  ...
  function toFeaturedItems(entries) {
    return entries.map((entry) => ({
      ...
      subtitle: [entry.author ?? "", entry.total_views ? `${formatCount(entry.total_views)} views` : ""].filter((part) => part.length > 0).join(" \u2022 ") || void 0
    }));
  }
  function toSearchResultItem(series) {
    return {
      ...
      title: series.title,
      imageUrl: series.thumbnail ?? "",
      subtitle: series.total_views ? `${formatCount(series.total_views)} views` : void 0,
      contentRating: "ADULT" /* ADULT */
    };
  }
  function parseTrending(response, range) {
    let parsed;
    try {
      parsed = JSON.parse(response);
    } catch {
  ...
  function toTrendingItems(entries) {
    return entries.map((entry, index) => {
      ...
      return {
        ...
        title: entry.title,
        imageUrl: entry.thumbnail ?? "",
        subtitle: [`#${index + 1}`, views ? `${formatCount(views)} views` : ""].filter((part) => part.length > 0).join(" \xB7 "),
        contentRating: "ADULT" /* ADULT */
      };
    });
  }
  ...
  var TempleScan = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
        domain: DOMAIN,
    ...
    async getSearchResultItems(query, metadata) {
      ...
      const matches = (await this.getDirectory()).filter((series) => {
        const matchesTitle = title.length === 0 || series.title.toLowerCase().includes(title) || (series.alternative_names ?? "").toLowerCase().includes(title);
        return matchesTitle && (status === void 0 || series.status === status);
      });
      ...
      return {
        ...
        metadata: page * PAGE_SIZE < sorted.length ? { page: page + 1 } : void 0
      };
    }
    async getMangaInfo(mangaId) {
      return toSourceManga2(parseSeriesData(await fetchSeriesPage(mangaId), mangaId), mangaId);
    }
    ...
    // -----------------------------------------------------------------
    getDirectory() {
      this.directory ??= fetchDirectory().then(parseDirectory).catch((error) => {
    ...
    getFeatured() {
      this.featured ??= Promise.all([fetchFeatured().then(parseFeatured), this.getDirectory()]).then(([entries, directory]) => toFeaturedItems(withFeaturedCovers(entries, directory))).catch((error) => {
  ...
  return __toCommonJS(TempleScan_exports);
})();
