...
var _Sources = (() => {
  ...
  // src/VyManga/VyManga.ts
  var VyManga_exports = {};
  __export(VyManga_exports, {
    VyManga: () => VyManga,
    VyMangaInfo: () => VyMangaInfo
  });
...
Please go to the homepage of <${hostOf(request.url)}> and press the cloud icon.`
  ...
  // src/VyManga/models.ts
  var DEFAULT_DOMAIN = "https://mangavyvy.net";
  var SEARCH_PATH = "search";
  var BASE_URL_KEY = "vymanga.baseUrlOverride";
  var GENRES_KEY = "vymanga.searchGenres";
  var SETTINGS_KEYS = [BASE_URL_KEY, GENRES_KEY];
  var SECTIONS = {
    POPULAR: "popular",
  ...
  var CARD_LINK_SELECTOR = "a";
  var CARD_TITLE_SELECTOR = ".comic-title";
  var CARD_IMAGE_SELECTOR = ".comic-image img, img.image, img.lozad";
  var CARD_LATEST_SELECTOR = ".comic-image > span, .comic-image span";
  var NEXT_PAGE_SELECTOR = "[rel=next]";
  var TITLE_SELECTOR = "h1";
  var THUMB_SELECTOR = ".img-manga img, .content-thumb img";
  var DESC_SELECTOR = ".summary > .content, div.summary p.content";
  var AUTHOR_SELECTOR = ".pre-title:contains(Author) ~ a";
  var ARTIST_SELECTOR = ".pre-title:contains(Artist) ~ a";
  var GENRE_SELECTOR = ".pre-title:contains(Genres) ~ a, div.col-md-7 p a[href*=genre]";
  var STATUS_SELECTOR = ".pre-title:contains(Status) ~ span:not(.space), div.col-md-7 p:contains(Status) span";
  var CHAPTER_SELECTOR = "a.list-chapter";
  var CHAPTER_FALLBACK_SELECTOR = 'a[id^="chapter-"]';
  var CHAPTER_DATE_SELECTOR = "p.small";
  var PAGE_SELECTOR = "div.carousel-item[data-page] img, img.lozad, img.d-block";
  var GENRE_OPTION_SELECTOR = ".checkbox-genre[data-value]";
  var FILTERS = {
    SORT: "sort",
    ORDER: "order",
    GENRE: "genre",
  ...
  var VyMangaInterceptor = class extends PaperbackInterceptor {
    ...
    async interceptResponse(request, response, body) {
      ...
      if (headerValue(response.headers, "cf-mitigated") === "challenge" || /(?:Just a moment|cf-chl-|_cf_chl_opt)/i.test(text)) {
  ...
  function buildBrowseUrl(baseUrl, sort, page) {
    return URL(baseUrl).addPathComponent(SEARCH_PATH).setQueryItem("sort", sort).setQueryItem("sort_type", "desc").setQueryItem("page", page).build();
  }
  function buildSearchUrl(baseUrl, request) {
    const url = URL(baseUrl).addPathComponent(SEARCH_PATH).setQueryItem("q", request.title ?? "").setQueryItem("page", request.page).setQueryItem("search_po", request.searchType ?? "0").setQueryItem("author_po", "0");
    ...
    if (request.sortBy !== void 0 || request.order !== void 0) {
      url.setQueryItem("sort_type", request.order ?? "desc");
    }
    if (request.includedGenres !== void 0 && request.includedGenres.length > 0) {
      url.setQueryItem("genre[]", request.includedGenres);
    }
    if (request.excludedGenres !== void 0 && request.excludedGenres.length > 0) {
      url.setQueryItem("exclude_genre[]", request.excludedGenres);
    }
    return url.build();
  }

  // src/VyManga/parsers.ts
  ...
  function extractMangaId(href) {
    return href.match(/\/manga\/([a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9])(?:[/?#]|$)/)?.[1] ?? href.match(/\/manga\/([a-zA-Z0-9-]+)/)?.[1];
  }
  function absoluteUrl(base, source) {
  ...
  function imageUrlFrom(base, image) {
    if (image.length === 0) {
      return "";
    }
    const source = image.attr("data-src") ?? image.attr("data-lazy-src") ?? image.attr("data-cfsrc") ?? image.attr("src") ?? "";
  ...
  function parseCard($, base, element) {
    ...
    return {
      ...
      title,
      imageUrl,
      subtitle: latest.length > 0 ? latest : void 0
    };
  }
  ...
  function parseGenres($) {
    ...
    for (const element of $(GENRE_OPTION_SELECTOR).toArray()) {
      ...
      if (id === void 0 || id.length === 0 || name.length === 0 || seen.has(id)) {
        continue;
      }
      seen.add(id);
  ...
  function parseMangaDetails($, base, mangaId, defaultRating) {
    ...
    return {
      ...
      mangaInfo: {
        ...
        contentRating: isAdult ? "ADULT" /* ADULT */ : defaultRating,
        tagGroups
      }
    };
  }
  function genreIdFromAnchor(href, title) {
  ...
  function parseChapters($, base, sourceManga) {
    ...
    for (const element of elements) {
      ...
      seen.add(chapterId);
      const title = Application.decodeHTMLEntities(
        row.find("span").first().text().trim() || row.find("p:not(.small)").first().text().trim() || row.text().trim()
      );
      chapters.push({
        chapterId,
        sourceManga,
  ...
  var LISTING_RATING = "MATURE" /* MATURE */;
  var VyManga = class extends PopmangoSource {
    constructor(cheerio) {
      super(cheerio, {
    ...
    async getDiscoverSections() {
      return [
        ...
        { id: SECTIONS.LATEST_UPDATES, title: "Latest Updates", type: "simpleCarousel" /* simpleCarousel */ },
  ...
  return __toCommonJS(VyManga_exports);
})();
